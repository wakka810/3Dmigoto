// Object			OS				DXGI version
// Feature level IDXGIDevice
// Win7			1.0				11.0 IDXGIDevice1
// Win7			1.0				11.0 IDXGIDevice2
// Platform update	1.2				11.1 IDXGIDevice3
// Win8.1			1.3 IDXGIDevice4
// 1.5
//
// IDXGIAdapter
// Win7			1.0				11.0 IDXGIAdapter1
// Win7			1.0				11.0 IDXGIAdapter2
// Platform update	1.2				11.1 IDXGIAdapter3
// 1.3
//
// IDXGIFactory
// Win7			1.0				11.0 IDXGIFactory1
// Win7			1.0				11.0 IDXGIFactory2
// Platform update	1.2				11.1 IDXGIFactory3
// Win8.1			1.3
// IDXGIFactory4					1.4
// IDXGIFactory5					1.5
//
// IDXGIOutput
// Win7			1.0				11.0 IDXGIOutput1
// Platform update	1.2				11.1 IDXGIOutput2
// Win8.1			1.3 IDXGIOutput3
// Win8.1			1.3 IDXGIOutput4
// Win10			1.4 IDXGIOutput5
// Win10			1.5
//
// IDXGIResource
// Win7			1.0				11.0 IDXGIResource1
// Platform update	1.2				11.1
//
// IDXGISwapChain
// Win7			1.0				11.0 IDXGISwapChain1
// Platform update	1.2				11.1 IDXGISwapChain2
// Win8.1			1.3 IDXGISwapChain3
// Win10			1.4
// IDXGISwapChain4					1.5
...
static BOOL(WINAPI *fnOrigSetWindowPos)(_In_ HWND hWnd,
                                        _In_opt_ HWND hWndInsertAfter,
                                        _In_ int X, _In_ int Y, _In_ int cx,
                                        _In_ int cy,
                                        _In_ UINT uFlags) = nullptr;

static BOOL WINAPI Hooked_SetWindowPos(_In_ HWND hWnd,
                                       _In_opt_ HWND hWndInsertAfter,
                                       _In_ int X, _In_ int Y, _In_ int cx,
                                       _In_ int cy, _In_ UINT uFlags) {
  if (G->SCREEN_UPSCALING != 0) {
    // Force desired upscaled resolution (only when desired resolution is
    // provided!)
    if (cx != 0 && cy != 0) {
      cx = G->SCREEN_WIDTH;
      cy = G->SCREEN_HEIGHT;
...
void InstallSetWindowPosHook() {
  ...
  fail |= InstallHookLate(hUser32, "SetWindowPos", (void **)&fnOrigSetWindowPos,
                          Hooked_SetWindowPos);
...
// In the Elite Dangerous case, they Release the HackerContext objects before
// creating the swap chain.  That causes problems, because we are not expecting
// anyone to get here without having a valid context.  They later call
// GetImmediateContext, which will generate a wrapped context.  So, since we
// need the context for our Overlay, let's do that a litte early in this case,
// which will save the reference for their GetImmediateContext call.

HackerSwapChain::HackerSwapChain(IDXGISwapChain1 *pSwapChain,
...
void HackerSwapChain::UpdateStereoParams() {
  ...
  // Update stereo parameter texture. It's possible to arrive here with no
  // texture available though, so we need to check first.
  if (mHackerDevice->mStereoTexture) {
    LogDebug("  updating stereo parameter texture.\n");
    mHackerDevice->mParamTextureManager.UpdateStereoTexture(
        mHackerDevice, mHackerContext, mHackerDevice->mStereoTexture, false);
  } else {
...
void HackerSwapChain::RunFrameActions() {
  ...
  // Regardless of log settings, since this runs every frame, let's flush the
  // log so that the most lost will be one frame worth.  Tradeoff of performance
  // to accuracy
  if (LogFile)
    fflush(LogFile);
  ...
  RunCommandList(mHackerDevice, mHackerContext, &G->present_command_list, NULL,
                 false);
  ...
  // is to make the arrays of shaders stable so that hunting up and down the
  // arrays is consistent, while the user is engaged.  After 1 minute, they are
  // likely onto some other spot, and we should start with a fresh set, to keep
  // the arrays and active shader list small for easier hunting.  Until the
  // first keypress, the arrays are cleared at each thread wake, just like
  // before. The arrays will be continually filled by the SetShader sections,
  // but should rapidly converge upon all active shaders.
...
// In the game Elex, we see them call do the unusual
// SwapChain->QueryInterface(SwapChain). We need to return This when that
// happens, because otherwise they disconnect us and we never get calls to
// Present.  Rather than do just this one-off, let's always return This for any
// time this might happen, as we've seen it happen in HackerContext too, for
// Mafia 3.  So any future instances cannot leak.
//
// From:
...
//client to determine 	whether two pointers point to the same component by
//calling QueryInterface with 	IID_IUnknown and comparing the results. 	It is
//specifically not the case that queries for interfaces other than IUnknown
//(even 	the same interface through the same pointer) must return the same
//pointer value.
//
STDMETHODIMP HackerSwapChain::QueryInterface(
    THIS_
        /* [in] */ REFIID riid,
  ...
  // Check for SwapChain2/3/4 passthrough BEFORE calling original QueryInterface
  // to avoid reference count leak when rejecting these interfaces
  const bool want_sc2 = (riid == __uuidof(IDXGISwapChain2));
  const bool want_sc3 = (riid == __uuidof(IDXGISwapChain3));
  const bool want_sc4 = (riid == __uuidof(IDXGISwapChain4));

  // SwapChain3 passthrough (default enabled to fix modern games)
  if (want_sc3 && G->allow_swapchain3_passthrough) {
    HRESULT hr = mOrigSwapChain1->QueryInterface(riid, ppvObject);
    LogInfo("  IDXGISwapChain3 passthrough -> hr=0x%08X, ptr=%p\n", hr,
            *ppvObject);
    return hr;
  }

  // SwapChain2/4 passthrough (configurable)
  if ((want_sc2 && G->allow_swapchain2_passthrough) ||
      (want_sc4 && G->allow_swapchain4_passthrough)) {
    HRESULT hr = mOrigSwapChain1->QueryInterface(riid, ppvObject);
    LogInfo("  IDXGISwapChain%d passthrough -> hr=0x%08X, ptr=%p\n",
            want_sc2 ? 2 : 4, hr, *ppvObject);
    return hr;
  }
  ...
  // For TheDivision, only upon Win10, it will request these.  Even though the
  // object we would return is the exact same pointer in memory, it still calls
  // into the object with a vtable entry that does not match what they expected.
  // Somehow they decide they are on Win10, and know these APIs ought to exist.
  // Does not crash on Win7.
  //
  // Returning an E_NOINTERFACE here seems to work, but this does call into
  // question our entire wrapping strategy.  If the object we've wrapped is a
  // superclass of the object they desire, the vtable is not going to match.
  //
  // FIX: Must Release before returning E_NOINTERFACE to prevent reference count
  // leak!
  ...
  IUnknown *unk_this;
  HRESULT hr_this =
      mOrigSwapChain1->QueryInterface(__uuidof(IUnknown), (void **)&unk_this);

  IUnknown *unk_ppvObject;
  HRESULT hr_ppvObject =
      reinterpret_cast<IUnknown *>(*ppvObject)
          ->QueryInterface(__uuidof(IUnknown), (void **)&unk_ppvObject);

  if (SUCCEEDED(hr_this) && SUCCEEDED(hr_ppvObject)) {
    // For an actual case of this->QueryInterface(this), just return our
    // HackerSwapChain object.
...
STDMETHODIMP
HackerSwapChain::SetPrivateDataInterface(THIS_
                                             /* [annotation][in] */
                                             __in REFGUID Name,
                                         /* [annotation][in] */
                                         __in const IUnknown *pUnknown) {
  LogInfo(
      "HackerSwapChain::SetPrivateDataInterface(%s@%p) called with GUID: %s\n",
      type_name(this), this, NameFromIID(Name).c_str());
...
STDMETHODIMP
HackerSwapChain::GetPrivateData(THIS_
                                    /* [annotation][in] */
                                    __in REFGUID Name,
                                /* [annotation][out][in] */
...
// More details:
// https://msdn.microsoft.com/en-us/library/windows/apps/hh465096.aspx
//
...
STDMETHODIMP HackerSwapChain::Present(THIS_
  ...
  if (!(Flags & DXGI_PRESENT_TEST)) {
    ...
    // which enables hunting, and snapshots, and aiming overrides and other
    // inputs
    RunFrameActions();
  ...
  get_tls()->hooking_quirk_protection =
      true; // Present may call D3D11CreateDevice, which we may have hooked
  ...
  if (!(Flags & DXGI_PRESENT_TEST)) {
    ...
    G->bb_is_upscaling_bb = !!G->SCREEN_UPSCALING &&
                            G->upscaling_command_list_using_explicit_bb_flip;
    ...
    RunCommandList(mHackerDevice, mHackerContext, &G->post_present_command_list,
                   NULL, true);
...
STDMETHODIMP
HackerSwapChain::SetFullscreenState(THIS_
                                        /* [in] */ BOOL Fullscreen,
                                    /* [annotation][in] */
  ...
  // if (pTarget)
  //	hr = mOrigSwapChain1->SetFullscreenState(Fullscreen,
  //pTarget->m_pOutput); else 	hr =
  //mOrigSwapChain1->SetFullscreenState(Fullscreen, 0);
...
STDMETHODIMP
HackerSwapChain::GetFullscreenState(THIS_
                                        /* [annotation][out] */
  ...
  //	if (ppTarget) LogInfo("  returns target IDXGIOutput = %x, wrapper =
  //%x\n", origOutput, *ppTarget);
  // }
...
STDMETHODIMP HackerSwapChain::ResizeTarget(
    THIS_
        /* [annotation][in] */
...
STDMETHODIMP
HackerSwapChain::GetContainingOutput(THIS_
                                         /* [annotation][out] */
...
STDMETHODIMP
HackerSwapChain::GetFrameStatistics(THIS_
                                        /* [annotation][out] */
...
STDMETHODIMP
HackerSwapChain::GetLastPresentCount(THIS_
                                         /* [annotation][out] */
...
STDMETHODIMP HackerSwapChain::GetFullscreenDesc(
    THIS_
        /* [annotation][out] */
  ...
  if (hr == S_OK) {
    ...
    if (pDesc)
      LogInfo("  returns Refresh rate = %f\n",
              (float)pDesc->RefreshRate.Numerator /
                  (float)pDesc->RefreshRate.Denominator);
  }
  LogInfo("  returns result = %x\n", hr);
...
STDMETHODIMP HackerSwapChain::Present1(
    THIS_
        /* [in] */ UINT SyncInterval,
  ...
  get_tls()->hooking_quirk_protection =
      true; // Present may call D3D11CreateDevice, which we may have hooked
  HRESULT hr =
      mOrigSwapChain1->Present1(SyncInterval, PresentFlags, pPresentParameters);
  get_tls()->hooking_quirk_protection = false;
...
STDMETHODIMP HackerSwapChain::GetRestrictToOutput(
    THIS_
        /* [annotation][out] */
...
STDMETHODIMP
HackerSwapChain::SetBackgroundColor(THIS_
                                        /* [annotation][in] */
...
STDMETHODIMP
HackerSwapChain::SetRotation(THIS_
                                 /* [annotation][in] */
...
STDMETHODIMP
HackerSwapChain::GetRotation(THIS_
                                 /* [annotation][out] */
...
HackerUpscalingSwapChain::HackerUpscalingSwapChain(
    IDXGISwapChain1 *pSwapChain, HackerDevice *pHackerDevice,
    HackerContext *pHackerContext, DXGI_SWAP_CHAIN_DESC *pFakeSwapChainDesc,
    UINT newWidth, UINT newHeight)
    : HackerSwapChain(pSwapChain, pHackerDevice, pHackerContext),
      mFakeBackBuffer(nullptr), mFakeSwapChain1(nullptr), mWidth(0),
      mHeight(0) {
...
void HackerUpscalingSwapChain::CreateRenderTarget(
  ...
  case 0: {
    ...
    hr = mHackerDevice->GetPassThroughOrigDevice1()->CreateTexture2D(
        &fake_buffer_desc, nullptr, &mFakeBackBuffer);
  ...
  case 1: {
    ...
    if (FAILED(hr)) {
      LogOverlay(LOG_DIRE, "HackerUpscalingSwapChain::createRenderTarget "
                           "failed to get DXGIFactory\n");
    ...
    pFactory->CreateSwapChain(mHackerDevice->GetPossiblyHookedOrigDevice1(),
                              pFakeSwapChainDesc, &swapChain);
    get_tls()->hooking_quirk_protection = false;
    ...
    // restore old state in case fall back is required ToDo: Unlikely needed
    // now.
  ...
  default:
    LogOverlay(LOG_DIRE,
               "*** HackerUpscalingSwapChain::HackerUpscalingSwapChain() "
               "failed ==> provided upscaling mode is not valid.\n");
    // Not positive if we will be able to get an overlay to
    // display the error, so also issue an audible warning:
    BeepFailure2();
    return;
...
STDMETHODIMP HackerUpscalingSwapChain::GetBuffer(THIS_
  ...
  if (mFakeBackBuffer) {
    ...
    LogInfo("BUG: HackerUpscalingDXGISwapChain::GetBuffer(): Missing upscaling "
            "object\n");
    DoubleBeepExit();
  }
...
STDMETHODIMP
HackerUpscalingSwapChain::SetFullscreenState(THIS_
                                                 /* [in] */ BOOL Fullscreen,
                                             /* [annotation][in] */
                                             _In_opt_ IDXGIOutput *pTarget) {
  ...
  // dont call setfullscreenstate again to avoid starting mode switching and
  // flooding winproc with unnecessary messages can disable fullscreen mode
  // somehow
  if (fullscreen_state && Fullscreen) {
    hr = S_OK;
  } else {
    if (G->SCREEN_UPSCALING == 2) {
      hr = mOrigSwapChain1->SetFullscreenState(
          TRUE, pTarget); // Witcher seems to require forcing the fullscreen
    } else {
...
STDMETHODIMP
HackerUpscalingSwapChain::GetDesc(THIS_
                                      /* [annotation][out] */
                                      _Out_ DXGI_SWAP_CHAIN_DESC *pDesc) {
  ...
  if (hr == S_OK) {
    if (pDesc) {
      // TODO: not sure whether the upscaled resolution or game resolution
      // should be returned
...
STDMETHODIMP
HackerUpscalingSwapChain::ResizeBuffers(THIS_
                                            /* [in] */ UINT BufferCount,
                                        /* [in] */ UINT Width,
                                        /* [in] */ UINT Height,
                                        /* [in] */ DXGI_FORMAT NewFormat,
  ...
  if (mFakeBackBuffer) // UPSCALE_MODE 0
  {
    ...
    // would my stuff work this way? i guess yes. What is with the games that
    // are not calling resize buffer just try to recreate texture with new game
    // resolution should be possible without any issues (texture just like the
    // swap chain should not be used at this time point)
    ...
    if (!(fd.Width == Width && fd.Height == Height)) {
      ...
      LockResourceCreationMode();
      hr = mHackerDevice->GetPassThroughOrigDevice1()->CreateTexture2D(
          &fd, nullptr, &mFakeBackBuffer);
      UnlockResourceCreationMode();
    } else // nothing to resize
      hr = S_OK;
  } else if (mFakeSwapChain1) // UPSCALE_MODE 1
  {
    // the last parameter have to be zero to avoid the influence of the faked
    // swap chain on the resize target function
    hr = mFakeSwapChain1->ResizeBuffers(BufferCount, Width, Height, NewFormat,
                                        0);
  } else {
    LogInfo("BUG: HackerUpscalingSwapChain::ResizeBuffers(): Missing upscaling "
            "object\n");
    DoubleBeepExit();
  }
...
STDMETHODIMP HackerUpscalingSwapChain::ResizeTarget(
    THIS_
        /* [annotation][in] */
        _In_ const DXGI_MODE_DESC *pNewTargetParameters) {
  ...
  if (pNewTargetParameters != nullptr) {
    // TODO: not sure if it belongs here, in the resize buffers function or in
    // both or maybe it is better to put it in the getviewport function? Require
    // in case the software mouse and upscaling are on at the same time
    G->GAME_INTERNAL_WIDTH = pNewTargetParameters->Width;
    G->GAME_INTERNAL_HEIGHT = pNewTargetParameters->Height;
  }

  // Some games like Witcher seems to drop fullscreen everytime the resizetarget
  // is called (original one) Some other games seems to require the function I
  // did it the way the faked texture mode (upscale_mode == 1) dont call resize
  // target the other mode does
  ...
  if (G->SCREEN_UPSCALING == 2) {
    ...
    LONG displ_chainge_res = ChangeDisplaySettingsEx(
        NULL, &dmScreenSettings, nullptr, CDS_FULLSCREEN, 0);
