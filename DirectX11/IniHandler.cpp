...
static Section RegularSections[] = {
    ...
    {L"Key", true},
    {L"Preset", true},
    {L"Include", true}, // Prefix so that it may be namespaced to allow included
                        // files to include more files with relative paths
    {L"Loader", false},
};
...
static bool whitelisted_duplicate_key(const wchar_t *section,
                                      const wchar_t *key) {
...
static bool IsCommandListSection(const wchar_t *section) {
  return SectionInList(section, CommandListSections,
                       ARRAYSIZE(CommandListSections));
}
...
static bool DoesSectionAllowLinesWithoutEquals(const wchar_t *section) {
  return SectionInList(section, AllowLinesWithoutEquals,
                       ARRAYSIZE(AllowLinesWithoutEquals)) ||
         IsCommandListSection(section);
}

static const wchar_t *SectionPrefixFromList(const wchar_t *section,
...
static const wchar_t *SectionPrefix(const wchar_t *section) {
  ...
  ret = SectionPrefixFromList(section, CommandListSections,
                              ARRAYSIZE(CommandListSections));
  if (!ret)
    ret = SectionPrefixFromList(section, RegularSections,
                                ARRAYSIZE(RegularSections));
  return ret;
}
...
typedef std::unordered_map<wstring, wstring, WStringInsensitiveHash,
                           WStringInsensitiveEquality>
    IniSectionMap;
typedef std::unordered_set<wstring, WStringInsensitiveHash,
                           WStringInsensitiveEquality>
    IniSectionSet;
...
// given prefix.
static IniSections::iterator prefix_upper_bound(IniSections &sections,
                                                wstring &prefix) {
...
static bool get_namespaced_section_name(const wstring *section,
...
bool get_namespaced_section_name_lower(const wstring *section,
                                       const wstring *ini_namespace,
                                       wstring *ret) {
  bool rc;
...
wstring get_namespaced_var_name_lower(const wstring var,
                                      const wstring *ini_namespace) {
  wstring ret =
      wstring(L"$\\") + *ini_namespace + wstring(L"\\") + var.substr(1);
  std::transform(ret.begin(), ret.end(), ret.begin(), ::towlower);
  return ret;
}

static bool _get_section_namespace(IniSections *custom_ini_sections,
                                   const wchar_t *section, wstring *ret) {
  try {
...
static bool _get_section_path(IniSections *custom_ini_sections,
                              const wchar_t *section, wstring *ret) {
  IniSection *entry;
...
static bool _get_namespaced_section_path(IniSections *custom_ini_sections,
                                         const wchar_t *section, wstring *ret) {
...
bool check_include_condition(wstring *val, const wstring *ini_namespace) {
  ...
  if (!condition.static_evaluate(&ret, NULL)) {
    IniWarning(
        "WARNING: Include condition could not be statically evaluated: %S\n",
        val->c_str());
    return false;
  }
...
static void ParseIniKeyValLine(wstring *wline, wstring *section,
  ...
  if (delim != wline->npos) {
    ...
    if (warn_duplicates == 2) {
      ...
      if ((warn_duplicates == 1) && !inserted &&
          !whitelisted_duplicate_key(section->c_str(), key.c_str())) {
...
static void ParseNamespacedIniFile(const wchar_t *ini,
                                   const wstring *ini_namespace) {
...
static void InsertBuiltInIniSections() {
  static const char text[] = "[BuiltInCustomShaderDisableScissorClipping]\n"
...
static pcre2_code *glob_to_regex(wstring &pattern) {
  ...
  if (pcre2_pattern_convert((PCRE2_SPTR)apattern.c_str(), apattern.length(),
                            PCRE2_CONVERT_GLOB, &converted, &blength, NULL)) {
    LogInfo("Bad pattern: exclude_recursive=%S\n", pattern.c_str());
    return NULL;
  }

  regex =
      pcre2_compile(converted, blength, PCRE2_CASELESS, &err, &err_off, NULL);
  if (!regex)
...
static vector<pcre2_code *>
globbing_vector_to_regex(vector<wstring> &globbing_patterns) {
...
static bool matches_globbing_vector(wchar_t *filename,
  ...
  for (pcre2_code *regex : patterns) {
    ...
    rc = pcre2_match(regex, (PCRE2_SPTR)afilename.c_str(),
...
static void ParseIniFilesRecursive(wchar_t *migoto_path,
  ...
  do {
    ...
    } else if (!wcscmp(find_data.cFileName + wcslen(find_data.cFileName) - 4,
                       L".ini")) {
...
static void _GetIniSection(IniSections *custom_ini_sections,
  ...
  try {
    *key_vals = &custom_ini_sections->at(section).kv_vec;
    ...
    LogDebug("WARNING: GetIniSection() called on a section not in the "
             "ini_sections map: %S\n",
             section);
...
bool GetIniString(const wchar_t *section, const wchar_t *key,
...
GetIniStringMultipleKeys(const wchar_t *section, const wchar_t *key) {
...
float GetIniFloat(const wchar_t *section, const wchar_t *key, float def,
                  bool *found) {
  wchar_t val[32];
  float ret = def;
  int len;
...
bool GetIniBool(const wchar_t *section, const wchar_t *key, bool def,
                bool *found, bool warn) {
  wchar_t val[32];
  bool ret = def;
  ...
  if (GetIniString(section, key, 0, val, 32)) {
    if (!_wcsicmp(val, L"1") || !_wcsicmp(val, L"true") ||
        !_wcsicmp(val, L"yes") || !_wcsicmp(val, L"on")) {
      LogInfo("  %S=1\n", key);
      if (found)
        *found = true;
      return true;
    }
    if (!_wcsicmp(val, L"0") || !_wcsicmp(val, L"false") ||
        !_wcsicmp(val, L"no") || !_wcsicmp(val, L"off")) {
      LogInfo("  %S=0\n", key);
      if (found)
        *found = true;
      return false;
...
static UINT64 GetIniHash(const wchar_t *section, const wchar_t *key, UINT64 def,
                         bool *found) {
  std::string val;
  UINT64 ret = def;
  int len;
...
static int GetIniHexString(const wchar_t *section, const wchar_t *key, int def,
                           bool *found) {
  std::string val;
  int ret = def;
  int len;
...
static int ParseEnum(wchar_t *str, wchar_t *prefix, wchar_t *names[],
                     int names_len, int first) {
  size_t prefix_len;
  wchar_t *ptr = str;
  int i;
...
static int GetIniEnum(const wchar_t *section, const wchar_t *key, int def,
                      bool *found, wchar_t *prefix, wchar_t *names[],
                      int names_len, int first) {
...
T GetIniEnumClass(const wchar_t *section, const wchar_t *key, T def,
                  bool *found,
...
T GetIniEnumClass(const wchar_t *section, const wchar_t *key, T def,
                  bool *found,
                  struct EnumName_t<const wchar_t *, T> *enum_names) {
  return GetIniEnumClass<const wchar_t *, T>(section, key, def, found,
                                             enum_names);
}
...
template <class T1, class T>
T GetIniEnumClass(const wchar_t *section, const wchar_t *key, T def,
                  bool *found, struct EnumName_t<const char *, T> *enum_names) {
  string val;
  T ret = def;
  bool tmp_found;
  ...
  if (GetIniString(section, key, 0, &val)) {
    ret = lookup_enum_val<const char *, T>(enum_names, val.c_str(), def,
                                           &tmp_found);
    if (tmp_found) {
      if (found)
        *found = tmp_found;
...
static int GetIniBoolOrInt(const wchar_t *section, const wchar_t *key, int def,
                           bool *found) {
  int ret;
  bool tmp_found;
...
static int GetIniBoolIntOrEnum(const wchar_t *section, const wchar_t *key,
                               int def, bool *found, wchar_t *prefix,
                               wchar_t *names[], int names_len, int first) {
  int ret;
  bool tmp_found;
...
static void GetUserConfigPath(const wchar_t *migoto_path) {
  ...
  rel_path =
      wstring(tmp.begin(), tmp.end()); // TODO: Sort out wide character mess
  if (tmp[1] != ':' && tmp[0] != '\\')
...
static void ParseIncludedIniFiles() {
  ...
  exclude = globbing_vector_to_regex(
      GetIniStringMultipleKeys(L"Include", L"exclude_recursive"));
  ...
  do {
    ...
    for (i = include_sections.begin(); i != include_sections.end(); i++) {
      ...
      _get_namespaced_section_path(&include_sections, i->first.c_str(),
                                   &namespace_path);
      ...
      for (entry = section->begin(); entry < section->end(); entry++) {
        ...
        // This is not a strong protection against including the same file
        // multiple times, but it is intended to ensure that this do while loop
...
static void RegisterPresetKeyBindings() {
  ...
  for (i = lower; i != upper; i++) {
    ...
    type = GetIniEnumClass(id, L"type", KeyOverrideType::ACTIVATE, NULL,
                           KeyOverrideTypeNames);

    delay = GetIniInt(id, L"delay", 0, NULL);
    release_delay = GetIniInt(id, L"release_delay", 0, NULL);

    if (type == KeyOverrideType::CYCLE) {
      shared_ptr<KeyOverrideCycle> cycle_preset =
          make_shared<KeyOverrideCycle>();
      shared_ptr<KeyOverrideCycleBack> cycle_back =
          make_shared<KeyOverrideCycleBack>(cycle_preset);
      preset = cycle_preset;
      for (wstring key : back)
        RegisterKeyBinding(L"Back", key.c_str(), cycle_back, 0, delay,
                           release_delay);
    } else {
...
static void EnumeratePresetOverrideSections() {
  ...
  for (i = lower; i != upper; i++) {
    ...
    preset_id = id;
    std::transform(preset_id.begin(), preset_id.end(), preset_id.begin(),
                   ::towlower);
...
static void ParsePresetOverrideSections() {
  ...
  for (i = begin(presetOverrides); i != end(presetOverrides); i++) {
    ...
    preset->ParseIniSection(id);
    preset->unique_triggers_required =
        GetIniInt(id, L"unique_triggers_required", 0, NULL);
  }
}

static char *type_to_format(float type) { return "%f%n"; }

static char *type_to_format(unsigned int type) { return "%u%n"; }

static char *type_to_format(signed int type) { return "%i%n"; }

static char *type_to_format(unsigned short type) { return "%hu%n"; }

static char *type_to_format(signed short type) { return "%hi%n"; }

static char *type_to_format(unsigned char type) { return "%hhu%n"; }

static char *type_to_format(signed char type) { return "%hhi%n"; }
...
template <typename T>
static void ConstructInitialData(CustomResource *custom_resource,
  ...
  memcpy(custom_resource->initial_data, vals.data(),
         custom_resource->initial_data_size);
}

static void ConstructInitialDataNorm(CustomResource *custom_resource,
                                     std::istringstream *tokens, int bytes,
                                     bool snorm) {
  ...
  for (i = 0; i < vals.size(); i++) {
    ...
    if (isnan(val)) {
      IniWarning(
          "WARNING: Special value unsupported as normalized integer: %f\n",
          val);
      val = 0;
    } else if (snorm) {
      if (val < -1.0 || val > 1.0)
...
static void ConstructInitialDataString(CustomResource *custom_resource,
                                       std::string *data) {
  ...
  case DXGI_FORMAT_R8_SINT:
    ...
    memcpy(custom_resource->initial_data, data->c_str() + 1,
           custom_resource->initial_data_size);
    return;
  default:
    IniWarning(
        "WARNING: unsupported format for specifying initial data as text\n");
    return;
  }
}

static void ParseResourceInitialData(CustomResource *custom_resource,
                                     const wchar_t *section) {
...
static void ParseResourceSections() {
  ...
  for (i = lower; i != upper; i++) {
    ...
    resource_id = i->first;
    std::transform(resource_id.begin(), resource_id.end(), resource_id.begin(),
                   ::towlower);
    ...
    if (GetIniStringAndLog(i->first.c_str(), L"filename", 0, setting,
                           MAX_PATH)) {
    ...
    custom_resource->override_type =
        GetIniEnumClass(i->first.c_str(), L"type", CustomResourceType::INVALID,
                        NULL, CustomResourceTypeNames);
    custom_resource->override_mode =
        GetIniEnumClass(i->first.c_str(), L"mode", CustomResourceMode::DEFAULT,
                        NULL, CustomResourceModeNames);
    ...
    custom_resource->override_width =
        GetIniInt(i->first.c_str(), L"width", -1, NULL);
    custom_resource->override_height =
        GetIniInt(i->first.c_str(), L"height", -1, NULL);
    custom_resource->override_depth =
        GetIniInt(i->first.c_str(), L"depth", -1, NULL);
    custom_resource->override_mips =
        GetIniInt(i->first.c_str(), L"mips", -1, NULL);
    custom_resource->override_array =
        GetIniInt(i->first.c_str(), L"array", -1, NULL);
    custom_resource->override_msaa =
        GetIniInt(i->first.c_str(), L"msaa", -1, NULL);
    custom_resource->override_msaa_quality =
        GetIniInt(i->first.c_str(), L"msaa_quality", -1, NULL);
    custom_resource->override_byte_width =
        GetIniInt(i->first.c_str(), L"byte_width", -1, NULL);
    custom_resource->override_stride =
        GetIniInt(i->first.c_str(), L"stride", -1, NULL);

    custom_resource->width_multiply =
        GetIniFloat(i->first.c_str(), L"width_multiply", 1.0f, NULL);
    custom_resource->height_multiply =
        GetIniFloat(i->first.c_str(), L"height_multiply", 1.0f, NULL);

    if (GetIniStringAndLog(i->first.c_str(), L"bind_flags", 0, setting,
                           MAX_PATH)) {
      custom_resource->override_bind_flags =
          parse_enum_option_string<const wchar_t *, CustomResourceBindFlags,
                                   wchar_t *>(CustomResourceBindFlagNames,
                                              setting, NULL);
    }

    if (GetIniStringAndLog(i->first.c_str(), L"misc_flags", 0, setting,
                           MAX_PATH)) {
      custom_resource->override_misc_flags =
          parse_enum_option_string<const wchar_t *, ResourceMiscFlags,
                                   wchar_t *>(ResourceMiscFlagNames, setting,
                                              NULL);
    }
...
static bool ParseCommandListLine(const wchar_t *ini_section, const wchar_t *lhs,
                                 wstring *rhs, wstring *raw_line,
                                 CommandList *command_list,
                                 CommandList *explicit_command_list,
                                 CommandList *pre_command_list,
                                 CommandList *post_command_list,
                                 const wstring *ini_namespace) {
  if (ParseCommandListGeneralCommands(ini_section, lhs, rhs,
  ...
  if (ParseCommandListVariableAssignment(ini_section, lhs, rhs, raw_line,
  ...
  if (raw_line && !explicit_command_list &&
      ParseCommandListFlowControl(ini_section, raw_line, pre_command_list,
                                  post_command_list, ini_namespace))
    return true;
...
static bool ParseCommandListLine(const wchar_t *ini_section, const wchar_t *lhs,
                                 const wchar_t *rhs, wstring *raw_line,
                                 CommandList *command_list,
                                 const wstring *ini_namespace) {
  wstring srhs = wstring(rhs);

  return ParseCommandListLine(ini_section, lhs, &srhs, raw_line, command_list,
                              command_list, NULL, NULL, ini_namespace);
}
...
static void ParseCommandList(const wchar_t *id, CommandList *pre_command_list,
                             CommandList *post_command_list,
                             wchar_t *whitelist[],
  ...
  if (!IsCommandListSection(id)) {
    LogInfoW(L"BUG: ParseCommandList() called on a section not in the "
             L"CommandListSections list: %s\n",
             id);
    DoubleBeepExit();
  }
  ...
  for (entry = section->begin(); entry < section->end(); entry++) {
    ...
    std::transform(raw_line->begin(), raw_line->end(), raw_line->begin(),
                   ::towlower);
    ...
    if (whitelist) {
      ...
      if (whitelist[i]) {
        ...
        if (whitelisted_keys.count(key->c_str())) {
          IniWarningW(L"WARNING: Duplicate non-command list key found "
                      L"in " INI_FILENAME L": [%ls] %ls\n",
                      id, key->c_str());
        }
    ...
    if (ParseCommandListLine(id, key_ptr, val, raw_line, command_list,
                             explicit_command_list, pre_command_list,
    ...
    if (entry->ini_namespace == G->user_config && !G->user_config.empty()) {
      ...
      if (!G->user_config_dirty) {
        LogOverlay(
            ...
            " Press %S to update the config now, or %S to reset all settings "
            "to default\n"
            " The first unrecognised entry was: \"%S\"\n",
            user_friendly_ini_key_binding(L"Hunting", L"reload_config").c_str(),
            user_friendly_ini_key_binding(L"Hunting", L"wipe_user_config")
                .c_str(),
            raw_line->c_str());
...
static void ParseConstantsSection() {
  ...
  for (next = section->begin(), entry = next; entry < section->end();
       entry = next) {
    next++;
    key = &entry->first;
    val = &entry->second;
    ...
    inserted = command_list_globals.emplace(
  ...
  ParseCommandList(L"Constants", &G->constants_command_list,
                   &G->post_constants_command_list, NULL);
}
...
static void check_shaderoverride_duplicates(bool duplicate, const wchar_t *id,
  ...
  allow_duplicates = GetIniBoolIntOrEnum(id, L"allow_duplicate_hash", 0, NULL,
                                         NULL, true_false_overrule,
                                         ARRAYSIZE(true_false_overrule), 0);
  ...
  if (duplicate && !allow_duplicates) {
    IniWarning(
        ...
        "[%S]\n"
        "[%S]\n"
        "If this is intentional, add allow_duplicate_hash=true or "
        "allow_duplicate_hash=overrule to suppress warning\n",
...
static void warn_deprecated_shaderoverride_options(const wchar_t *id,
                                                   ShaderOverride *override) {
  ...
  if (override->partner_hash &&
      (!override->command_list.commands.empty() ||
       !override->post_command_list.commands.empty())) {
    LogOverlay(LOG_NOTICE,
               "WARNING: [%S] tried to combine the deprecated partner= option "
               "with a command list.\n"
               "This almost certainly won't do what you want. Try something "
               "like this instead:\n"
               "\n"
               "[%S_VERTEX_SHADER]\n"
  ...
  if (override->depth_filter != DepthBufferFilter::NONE) {
    LogOverlay(LOG_NOTICE,
               "NOTICE: [%S] used deprecated depth_filter option. Consider "
               "texture filtering for more flexibility:\n"
               "\n"
               "[%S]\n"
               "x = oD\n"
               "\n"
               ...
               "ld_indexable(texture1d)(float,float,float,float) r0.x, l(0, 0, "
               "0, 0), t120.xyzw\n"
...
wchar_t *ShaderOverrideIniKeys[] = {
    L"hash",  L"allow_duplicate_hash", L"depth_filter", L"partner",
    L"model", L"disable_scissor",      L"filter_index", NULL};
static void ParseShaderOverrideSections() {
  ...
  //  We actually already lock the entire config reload, so this is redundant
  //  -DSS
  ...
  for (i = lower; i != upper; i++) {
    ...
    override->depth_filter =
        GetIniEnumClass(id, L"depth_filter", DepthBufferFilter::NONE, NULL,
                        DepthBufferFilterNames);
    ...
    ParseCommandList(id, &override->command_list, &override->post_command_list,
                     ShaderOverrideIniKeys);
    ...
    if (found) {
      ...
      if (disable_scissor)
        ParseCommandListLine(id, L"run",
                             L"builtincustomshaderdisablescissorclipping", NULL,
                             &override->command_list, &ini_namespace);
      else
        ParseCommandListLine(id, L"run",
                             L"builtincustomshaderenablescissorclipping", NULL,
                             &override->command_list, &ini_namespace);
    }
...
template <typename T> static std::set<T> vec_to_set(std::vector<T> &v) {
...
wchar_t *ShaderRegexIniKeys[] = {
    ...
    NULL};
static bool parse_shader_regex_section_main(const std::wstring *section_id,
                                            ShaderRegexGroup *regex_group) {
  std::string setting;
  std::vector<std::string> items;

  if (!GetIniStringAndLog(section_id->c_str(), L"shader_model", NULL,
                          &setting)) {
  ...
  regex_group->filter_index =
      GetIniFloat(section_id->c_str(), L"filter_index", FLT_MAX, NULL);

  ParseCommandList(section_id->c_str(), &regex_group->command_list,
                   &regex_group->post_command_list, ShaderRegexIniKeys);
  return true;
}

static bool parse_shader_regex_section_pattern(const std::wstring *section_id,
  ...
  // \n or \s+ easier to spot.
  LogInfo(
      "--------- final pcre2 regex pattern used after ini parsing ---------\n");
  LogInfo("%s\n", pattern.c_str());
  LogInfo(
      "--------------------------------------------------------------------\n");
...
static bool
parse_shader_regex_section_declarations(const std::wstring *section_id,
                                        const std::wstring *pattern_id,
                                        ShaderRegexGroup *regex_group) {
  IniSectionVector *section = NULL;
  IniSectionVector::iterator entry;
...
static bool parse_shader_regex_section_replace(const std::wstring *section_id,
                                               const std::wstring *pattern_id,
                                               ShaderRegexGroup *regex_group) {
  IniSectionVector *section = NULL;
  IniSectionVector::iterator entry;
...
static ShaderRegexGroup *get_regex_group(std::wstring *regex_id,
                                         bool allow_creation) {
  if (allow_creation)
...
static void delete_regex_group(std::wstring *regex_id) {
  ...
  if (i != shader_regex_groups.end()) {
    auto it =
        std::find(registered_command_lists.begin(),
                  registered_command_lists.end(), &i->second.command_list);
    if (it != registered_command_lists.end())
      registered_command_lists.erase(it);
    it =
        std::find(registered_command_lists.begin(),
                  registered_command_lists.end(), &i->second.post_command_list);
    if (it != registered_command_lists.end())
      registered_command_lists.erase(it);
...
static void ParseShaderRegexSections() {
  ...
  hash = crc32c_hw(hash, &G->assemble_signature_comments,
                   sizeof(G->assemble_signature_comments));
  hash = crc32c_hw(hash, &G->disassemble_undecipherable_custom_data,
                   sizeof(G->disassemble_undecipherable_custom_data));
  ...
  for (i = lower; i != upper; i++) {
    ...
    regex_group =
        get_regex_group(&subsection_names[0], subsection_names.size() == 1);
    if (!regex_group)
      continue;
    ...
    case 2:
      if (!_wcsicmp(subsection_names[1].c_str(), L"Pattern")) {
        ...
        if (parse_shader_regex_section_pattern(section_id, &subsection_names[1],
                                               regex_group))
          continue;
      } else if (!_wcsicmp(subsection_names[1].c_str(),
                           L"InsertDeclarations")) {
        if (parse_shader_regex_section_declarations(
                section_id, &subsection_names[1], regex_group))
          continue;
      }
      break;
    case 3:
      if (!_wcsnicmp(subsection_names[1].c_str(), L"Pattern", 7) &&
        ...
        if (parse_shader_regex_section_replace(section_id, &subsection_names[1],
                                               regex_group))
          continue;
      }
      break;
...
#define TEXTURE_OVERRIDE_FUZZY_MATCHES                                         \
  L"match_type", L"match_usage", L"match_bind_flags",                          \
      L"match_cpu_access_flags", L"match_misc_flags", L"match_byte_width",     \
      L"match_stride", L"match_mips", L"match_format", L"match_width",         \
      L"match_height", L"match_depth", L"match_array", L"match_msaa",          \
...
#define TEXTURE_OVERRIDE_DRAW_CALL_MATCHES                                     \
  L"match_first_vertex", L"match_first_index", L"match_first_instance",        \
      L"match_vertex_count", L"match_index_count", L"match_instance_count"

// List of keys in [TextureOverride] sections that are processed in this
// function. Used by ParseCommandList to find any unrecognised lines.
wchar_t *TextureOverrideIniKeys[] = {L"hash",
                                     L"stereomode",
                                     L"format",
                                     L"width",
                                     L"height",
...
wchar_t *TextureOverrideFuzzyMatchesIniKeys[] = {TEXTURE_OVERRIDE_FUZZY_MATCHES,
                                                 NULL};
...
static void parse_fuzzy_numeric_match_expression_error(const wchar_t *text) {
  IniWarning(
      ...
      "    [ operator ] value | field_name [ * field_name ] [ * multiplier ] [ "
      "/ divider ]\n"
      "    Parse error on text: \"%S\"\n",
      text);
}

static bool parse_fuzzy_field_name(const wchar_t **ptr,
...
static void parse_fuzzy_numeric_match_expression(const wchar_t *setting,
                                                 FuzzyMatch *matcher) {
  ...
  //
  //     operator   =   "=" | "!" | "<" | ">" | "<=" | ">="
  //     field_name =   "width" | "height" | "depth" | "array" | "res_width" |
  //     "res_height" value, multiplier and divider are integers.
  //
...
static void parse_texture_override_common(const wchar_t *id,
  ...
  if (GetIniString(id, L"Iteration", 0, setting, MAX_PATH)) {
    ...
    swscanf_s(setting, L"%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", id + 0, id + 1, id + 2,
  ...
  override->expand_region_copy =
      GetIniBool(id, L"expand_region_copy", false, NULL);
  ...
  if (GetIniStringAndLog(id, L"match_first_vertex", 0, setting, MAX_PATH)) {
    parse_fuzzy_numeric_match_expression(setting,
                                         &override->match_first_vertex);
  ...
  if (GetIniStringAndLog(id, L"match_instance_count", 0, setting, MAX_PATH)) {
    parse_fuzzy_numeric_match_expression(setting,
                                         &override->match_instance_count);
    override->has_draw_context_match = true;
  }

  ParseCommandList(id, &override->command_list, &override->post_command_list,
                   TextureOverrideIniKeys, register_command_lists);
}

static bool
texture_override_section_has_fuzzy_match_keys(const wchar_t *section) {
  int i;
...
template <class T>
static bool
parse_masked_flags_field(const wstring setting, unsigned *val, unsigned *mask,
                         struct EnumName_t<const wchar_t *, T> *enum_names) {
  ...
  if (ret != 0 && ret != EOF &&
      (len1 == setting.length() || len2 == setting.length())) {
    if (ret == 2)
      *mask = 0xffffffff;
    LogInfo("    Using: 0x%08x / 0x%08x\n", *val, *mask);
    return true;
  ...
  *mask = 0;
  ...
  for (i = 0; i < tokens.size(); i++) {
    ...
    tmp = (unsigned)lookup_enum_val<const wchar_t *, T>(enum_names,
                                                        token.c_str(), (T)0);
...
static void parse_texture_override_fuzzy_match(const wchar_t *section) {
  ...
  ival = GetIniEnum(section, L"match_type", D3D11_RESOURCE_DIMENSION_UNKNOWN,
                    &found, L"D3D11_RESOURCE_DIMENSION_", ResourceDimensions,
  ...
  ival =
      GetIniEnum(section, L"match_usage", D3D11_USAGE_DEFAULT, &found,
                 L"D3D11_USAGE_", ResourceUsage, ARRAYSIZE(ResourceUsage), 0);
  fuzzy->Usage.op = FuzzyMatchOp::EQUAL;
  fuzzy->Usage.val = ival;

  // Flags
  if (GetIniStringAndLog(section, L"match_bind_flags", 0, setting, MAX_PATH)) {
    if (parse_masked_flags_field(setting, &fuzzy->BindFlags.val,
                                 &fuzzy->BindFlags.mask,
                                 CustomResourceBindFlagNames)) {
      fuzzy->BindFlags.op = FuzzyMatchOp::EQUAL;
    }
  }
  if (GetIniStringAndLog(section, L"match_cpu_access_flags", 0, setting,
                         MAX_PATH)) {
    if (parse_masked_flags_field(setting, &fuzzy->CPUAccessFlags.val,
                                 &fuzzy->CPUAccessFlags.mask,
  ...
  if (GetIniStringAndLog(section, L"match_misc_flags", 0, setting, MAX_PATH)) {
    if (parse_masked_flags_field(setting, &fuzzy->MiscFlags.val,
                                 &fuzzy->MiscFlags.mask,
                                 ResourceMiscFlagNames)) {
  ...
  if (!G->mFuzzyTextureOverrides
           .insert(std::shared_ptr<FuzzyMatchResourceDesc>(fuzzy))
           .second) {
...
static void warn_if_duplicate_texture_hash(TextureOverride *override,
                                           uint32_t hash) {
  ...
  for (j = i->second.begin(); j != i->second.end(); j++) {
    ...
    IniWarning(
        ...
        "[%S]\n"
        "[%S]\n"
        "If this is intentional, add a match_priority=n to suppress warning "
        "and disambiguate order\n",
...
static void ParseBlendOp(wchar_t *key, wchar_t *val, D3D11_BLEND_OP *op,
                         D3D11_BLEND *src, D3D11_BLEND *dst) {
  wchar_t op_buf[32], src_buf[32], dst_buf[32];
  int i;

  i = swscanf_s(val, L"%s %s %s", op_buf, (unsigned)ARRAYSIZE(op_buf), src_buf,
                (unsigned)ARRAYSIZE(src_buf), dst_buf,
  ...
  try {
    *op = (D3D11_BLEND_OP)ParseEnum(op_buf, L"D3D11_BLEND_OP_", BlendOPs,
                                    ARRAYSIZE(BlendOPs), 1);
  } catch (EnumParseError) {
    IniWarning("WARNING: Unrecognised blend operation %S\n", op_buf);
  }

  try {
    *src = (D3D11_BLEND)ParseEnum(src_buf, L"D3D11_BLEND_", BlendFactors,
                                  ARRAYSIZE(BlendFactors), 1);
  } catch (EnumParseError) {
    IniWarning("WARNING: Unrecognised blend source factor %S\n", src_buf);
  }

  try {
    *dst = (D3D11_BLEND)ParseEnum(dst_buf, L"D3D11_BLEND_", BlendFactors,
                                  ARRAYSIZE(BlendFactors), 1);
  } catch (EnumParseError) {
...
static bool ParseBlendRenderTarget(D3D11_RENDER_TARGET_BLEND_DESC *desc,
  ...
  if (GetIniStringAndLog(section, key, 0, setting, MAX_PATH)) {
    ...
    ParseBlendOp(key, setting, &desc->BlendOp, &desc->SrcBlend,
                 &desc->DestBlend);
  ...
  if (GetIniStringAndLog(section, key, 0, setting, MAX_PATH)) {
    override = true;
    ParseBlendOp(key, setting, &desc->BlendOpAlpha, &desc->SrcBlendAlpha,
                 &desc->DestBlendAlpha);
  ...
  desc->RenderTargetWriteMask =
      GetIniHexString(section, key, D3D11_COLOR_WRITE_ENABLE_ALL, &found);
  if (found) {
    override = true;
...
static void ParseBlendState(CustomShader *shader, const wchar_t *section) {
  ...
  if (ParseBlendRenderTarget(&desc->RenderTarget[0], &mask->RenderTarget[0],
                             section, -1))
    shader->blend_override = 1;
  for (i = 1; i < 8; i++) {
    memcpy(&desc->RenderTarget[i], &desc->RenderTarget[0],
           sizeof(D3D11_RENDER_TARGET_BLEND_DESC));
    memcpy(&mask->RenderTarget[i], &mask->RenderTarget[0],
           sizeof(D3D11_RENDER_TARGET_BLEND_DESC));
  }
  ...
  for (i = 0; i < 8; i++) {
    if (ParseBlendRenderTarget(&desc->RenderTarget[i], &mask->RenderTarget[i],
                               section, i)) {
      shader->blend_override = 1;
  ...
  desc->AlphaToCoverageEnable =
      GetIniBool(section, L"alpha_to_coverage", false, &found);
  if (found) {
    shader->blend_override = 1;
  ...
  shader->blend_sample_mask =
      GetIniHexString(section, L"sample_mask", 0xffffffff, &found);
  if (found) {
    shader->blend_override = 1;
...
static wchar_t *ComparisonFuncs[] = {
    L"",        L"NEVER",     L"LESS",          L"EQUAL",  L"LESS_EQUAL",
    L"GREATER", L"NOT_EQUAL", L"GREATER_EQUAL", L"ALWAYS",
};

// https://msdn.microsoft.com/en-us/library/windows/desktop/ff476219(v=vs.85).aspx
static wchar_t *StencilOps[] = {
    L"",         L"KEEP",   L"ZERO", L"REPLACE", L"INCR_SAT",
    L"DECR_SAT", L"INVERT", L"INCR", L"DECR",
};

static void ParseStencilOp(wchar_t *key, wchar_t *val,
                           D3D11_DEPTH_STENCILOP_DESC *desc) {
  wchar_t func_buf[32], both_pass_buf[32], depth_fail_buf[32],
      stencil_fail_buf[32];
  int i;

  i = swscanf_s(val, L"%s %s %s %s", func_buf, (unsigned)ARRAYSIZE(func_buf),
  ...
  try {
    desc->StencilFunc = (D3D11_COMPARISON_FUNC)ParseEnum(
        func_buf, L"D3D11_COMPARISON_", ComparisonFuncs,
        ARRAYSIZE(ComparisonFuncs), 1);
  } catch (EnumParseError) {
  ...
  try {
    desc->StencilPassOp =
        (D3D11_STENCIL_OP)ParseEnum(both_pass_buf, L"D3D11_STENCIL_OP_",
                                    StencilOps, ARRAYSIZE(StencilOps), 1);
  } catch (EnumParseError) {
  ...
  try {
    desc->StencilDepthFailOp =
        (D3D11_STENCIL_OP)ParseEnum(depth_fail_buf, L"D3D11_STENCIL_OP_",
                                    StencilOps, ARRAYSIZE(StencilOps), 1);
  } catch (EnumParseError) {
  ...
  try {
    desc->StencilFailOp =
        (D3D11_STENCIL_OP)ParseEnum(stencil_fail_buf, L"D3D11_STENCIL_OP_",
                                    StencilOps, ARRAYSIZE(StencilOps), 1);
  } catch (EnumParseError) {
...
static void ParseDepthStencilState(CustomShader *shader,
                                   const wchar_t *section) {
  ...
  desc->DepthWriteMask = (D3D11_DEPTH_WRITE_MASK)GetIniEnum(
      section, L"depth_write_mask", D3D11_DEPTH_WRITE_MASK_ALL, &found,
  ...
  desc->DepthFunc = (D3D11_COMPARISON_FUNC)GetIniEnum(
      section, L"depth_func", D3D11_COMPARISON_LESS, &found,
  ...
  desc->StencilReadMask = GetIniHexString(
      section, L"stencil_read_mask", D3D11_DEFAULT_STENCIL_READ_MASK, &found);
  if (found) {
    shader->depth_stencil_override = 1;
    mask->StencilReadMask = 0;
  }

  desc->StencilWriteMask = GetIniHexString(
      section, L"stencil_write_mask", D3D11_DEFAULT_STENCIL_WRITE_MASK, &found);
  if (found) {
    shader->depth_stencil_override = 1;
...
static void ParseRSState(CustomShader *shader, const wchar_t *section) {
  ...
  desc->FillMode = (D3D11_FILL_MODE)GetIniEnum(
      section, L"fill", D3D11_FILL_SOLID, &found, L"D3D11_FILL_", FillModes,
      ARRAYSIZE(FillModes), 2);
  if (found) {
    shader->rs_override = 1;
    mask->FillMode = (D3D11_FILL_MODE)0;
  }

  desc->CullMode = (D3D11_CULL_MODE)GetIniEnum(
      section, L"cull", D3D11_CULL_BACK, &found, L"D3D11_CULL_", CullModes,
      ARRAYSIZE(CullModes), 1);
  if (found) {
    shader->rs_override = 1;
  ...
  desc->FrontCounterClockwise =
      (BOOL)GetIniEnum(section, L"front", 0, &found, NULL, FrontDirection,
                       ARRAYSIZE(FrontDirection), 0);
  if (found) {
    shader->rs_override = 1;
  ...
  desc->SlopeScaledDepthBias =
      GetIniFloat(section, L"slope_scaled_depth_bias", 0, &found);
  if (found) {
    shader->rs_override = 1;
  ...
  desc->DepthClipEnable =
      GetIniBool(section, L"depth_clip_enable", true, &found);
  if (found) {
    shader->rs_override = 1;
  ...
  desc->MultisampleEnable =
      GetIniBool(section, L"multisample_enable", false, &found);
  if (found) {
    shader->rs_override = 1;
  ...
  desc->AntialiasedLineEnable =
      GetIniBool(section, L"antialiased_line_enable", false, &found);
  if (found) {
    shader->rs_override = 1;
...
static void ParseSamplerState(CustomShader *shader, const wchar_t *section) {
  ...
  // TODO: do not really understand the difference between normal and comparison
  // filter
  ...
  if (GetIniStringAndLog(section, L"sampler", 0, setting, MAX_PATH)) {
    ...
    if (!_wcsicmp(setting, L"anisotropic_filter")) {
      desc->Filter = D3D11_FILTER_COMPARISON_ANISOTROPIC;
      desc->MaxAnisotropy = 16; // TODO: is 16 necessary or maybe it should be
                                // provided by the config ini?
      shader->sampler_override = 1;
      return;
    }
...
wchar_t *CustomShaderIniKeys[] = {
    L"vs", L"hs", L"ds", L"gs", L"ps", L"cs", L"max_executions_per_frame",
    L"flags",
    // OM Blend State overrides:
    L"blend", L"alpha", L"mask", L"blend[0]", L"blend[1]", L"blend[2]",
    L"blend[3]", L"blend[4]", L"blend[5]", L"blend[6]", L"blend[7]",
    L"alpha[0]", L"alpha[1]", L"alpha[2]", L"alpha[3]", L"alpha[4]",
    L"alpha[5]", L"alpha[6]", L"alpha[7]", L"mask[0]", L"mask[1]", L"mask[2]",
    L"mask[3]", L"mask[4]", L"mask[5]", L"mask[6]", L"mask[7]",
    L"alpha_to_coverage", L"sample_mask", L"blend_factor[0]",
    L"blend_factor[1]", L"blend_factor[2]", L"blend_factor[3]",
    L"blend_state_merge",
    // OM Depth Stencil State overrides:
    L"depth_enable", L"depth_write_mask", L"depth_func", L"stencil_enable",
    L"stencil_read_mask", L"stencil_write_mask", L"stencil_front",
    L"stencil_back", L"stencil_ref", L"depth_stencil_state_merge",
    // RS State overrides:
    ...
    L"sampler", // TODO: add additional sampler parameter
                // For now due to the lack of sampler as a custom resource only
                // filtering is added no further parameter are implemented
    NULL};
static void _EnumerateCustomShaderSections(IniSections::iterator lower,
  ...
  for (i = lower; i != upper; i++) {
    ...
    // consistent in the unordered_map:
    shader_id = i->first;
    std::transform(shader_id.begin(), shader_id.end(), shader_id.begin(),
                   ::towlower);
...
static void ParseCustomShaderSections() {
  ...
  for (i = customShaders.begin(); i != customShaders.end(); i++) {
    ...
    if (GetIniStringAndLog(shader_id->c_str(), L"flags", 0, setting,
                           MAX_PATH)) {
      custom_shader->compile_flags =
          parse_enum_option_string<const wchar_t *, D3DCompileFlags, wchar_t *>(
    ...
    if (GetIniString(shader_id->c_str(), L"vs", 0, setting, MAX_PATH))
      failed |=
          custom_shader->compile('v', setting, shader_id, &namespace_path);
    if (GetIniString(shader_id->c_str(), L"hs", 0, setting, MAX_PATH))
      failed |=
          custom_shader->compile('h', setting, shader_id, &namespace_path);
    if (GetIniString(shader_id->c_str(), L"ds", 0, setting, MAX_PATH))
      failed |=
          custom_shader->compile('d', setting, shader_id, &namespace_path);
    if (GetIniString(shader_id->c_str(), L"gs", 0, setting, MAX_PATH))
      failed |=
          custom_shader->compile('g', setting, shader_id, &namespace_path);
    if (GetIniString(shader_id->c_str(), L"ps", 0, setting, MAX_PATH))
      failed |=
          custom_shader->compile('p', setting, shader_id, &namespace_path);
    if (GetIniString(shader_id->c_str(), L"cs", 0, setting, MAX_PATH))
      failed |=
          custom_shader->compile('c', setting, shader_id, &namespace_path);
    ...
    ParseCommandList(shader_id->c_str(), &custom_shader->command_list,
                     &custom_shader->post_command_list, CustomShaderIniKeys);
  }
}
...
static void _EnumerateExplicitCommandListSections(IniSections::iterator lower,
                                                  IniSections::iterator upper) {
  IniSections::iterator i;
  wstring section_id;
  ...
  for (i = lower; i != upper; i++) {
    ...
    // consistent in the unordered_map:
    section_id = i->first;
    std::transform(section_id.begin(), section_id.end(), section_id.begin(),
                   ::towlower);
...
static void ParseExplicitCommandListSections() {
  ...
  for (i = explicitCommandListSections.begin();
       i != explicitCommandListSections.end(); i++) {
    section_id = &i->first;
    ...
    ParseCommandList(section_id->c_str(), &command_list_section->command_list,
                     &command_list_section->post_command_list, NULL);
  }
}
...
static void ToggleFullScreen(HackerDevice *device, void *private_data) {
  ...
  LogInfo("> full screen forcing toggled to %d (will not take effect until "
          "next mode switch)\n",
          G->SCREEN_FULLSCREEN);
}
...
static void ForceFullScreen(HackerDevice *device, void *private_data) {
  ...
  if (!mHackerSwapChain) {
    LogOverlay(LOG_DIRE,
               "force_full_screen_on_key: Unable to find swap chain\n");
    return;
  }
...
static void warn_of_conflicting_d3dx(wchar_t *dll_ini_path) {
  ...
  LogOverlay(LOG_WARNING,
             "Detected a conflicting d3dx.ini in the game directory that is "
             "not being used.\n"
...
void LoadConfigFile() {
  ...
  // Unbuffered logging to remove need for fflush calls, and r/w access to make
  // it easy to open active files.
  ...
  // Set the CPU affinity based upon d3dx.ini setting.  Useful for debugging and
  // shader hunting in AC3.
  ...
  G->dump_all_profiles =
      GetIniBool(L"Logging", L"dump_all_profiles", false, NULL);
  ...
  G->load_library_redirect =
      GetIniInt(L"System", L"load_library_redirect", 2, NULL);
  ...
  if (GetIniStringAndLog(L"System", L"hook", 0, setting, MAX_PATH)) {
    ...
    if (G->enable_hooks & EnableHooks::DEPRECATED)
      LogOverlay(LOG_NOTICE, "Deprecated hook options: Please remove "
                             "\"except\" and \"skip\" options\n");
  }
  G->enable_check_interface =
      GetIniBool(L"System", L"allow_check_interface", false, NULL);
  G->enable_create_device =
      GetIniInt(L"System", L"allow_create_device", 0, NULL);
  G->enable_platform_update =
      GetIniBool(L"System", L"allow_platform_update", false, NULL);
  G->allow_swapchain2_passthrough =
      GetIniBool(L"System", L"allow_swapchain2_passthrough", false, NULL);
  G->allow_swapchain3_passthrough =
      GetIniBool(L"System", L"allow_swapchain3_passthrough", true, NULL);
  G->allow_swapchain4_passthrough =
      GetIniBool(L"System", L"allow_swapchain4_passthrough", false, NULL);
  // TODO: Enable this by default if wider testing goes well:
  G->check_foreground_window =
      GetIniBool(L"System", L"check_foreground_window", false, NULL);
  ...
  if (GetIniStringAndLog(L"Device", L"filter_refresh_rate", 0, setting,
                         MAX_PATH)) {
    swscanf_s(
        setting, L"%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", G->FILTER_REFRESH + 0,
        G->FILTER_REFRESH + 1, G->FILTER_REFRESH + 2, G->FILTER_REFRESH + 3,
        G->FILTER_REFRESH + 4, G->FILTER_REFRESH + 5, G->FILTER_REFRESH + 6,
        G->FILTER_REFRESH + 7, G->FILTER_REFRESH + 8, G->FILTER_REFRESH + 9);
  }

  G->SCREEN_FULLSCREEN = GetIniInt(L"Device", L"full_screen", -1, NULL);
  RegisterIniKeyBinding(L"Device", L"toggle_full_screen", ToggleFullScreen,
                        NULL, 0, NULL);
  RegisterIniKeyBinding(L"Device", L"force_full_screen_on_key", ForceFullScreen,
                        NULL, 0, NULL);
  G->gForceStereo = GetIniInt(L"Device", L"force_stereo", 0, NULL);
  G->SCREEN_ALLOW_COMMANDS =
      GetIniBool(L"Device", L"allow_windowcommands", false, NULL);

  G->mResolutionInfo.from =
      GetIniEnumClass(L"Device", L"get_resolution_from",
                      GetResolutionFrom::INVALID, NULL, GetResolutionFromNames);

  G->hide_cursor = GetIniBool(L"Device", L"hide_cursor", false, NULL);
  G->cursor_upscaling_bypass =
      GetIniBool(L"Device", L"cursor_upscaling_bypass", true, NULL);

  // [Stereo]
  LogInfo("[Stereo]\n");
  bool automaticMode =
      GetIniBool(L"Stereo", L"automatic_mode", false, NULL); // in NVapi dll
  G->gCreateStereoProfile =
      GetIniBool(L"Stereo", L"create_profile", false, NULL);
  G->gSurfaceCreateMode = GetIniInt(L"Stereo", L"surface_createmode", -1, NULL);
  G->gSurfaceSquareCreateMode =
      GetIniInt(L"Stereo", L"surface_square_createmode", -1, NULL);
  G->gForceNoNvAPI = GetIniBool(L"Stereo", L"force_no_nvapi", false, NULL);

  // [Rendering]
  LogInfo("[Rendering]\n");

  G->shader_hash_type = GetIniEnumClass(
      L"Rendering", L"shader_hash", ShaderHashType::FNV, NULL, ShaderHashNames);
  G->texture_hash_version = GetIniInt(L"Rendering", L"texture_hash", 0, NULL);

  if (GetIniStringAndLog(L"Rendering", L"override_directory", 0, G->SHADER_PATH,
                         MAX_PATH)) {
  ...
  if (GetIniStringAndLog(L"Rendering", L"cache_directory", 0,
                         G->SHADER_CACHE_PATH, MAX_PATH)) {
  ...
  G->SCISSOR_DISABLE =
      GetIniBool(L"Rendering", L"rasterizer_disable_scissor", false, NULL);
  G->track_texture_updates =
      GetIniBoolOrInt(L"Rendering", L"track_texture_updates", 0, NULL);
  G->assemble_signature_comments =
      GetIniBool(L"Rendering", L"assemble_signature_comments", false, NULL);
  G->disassemble_undecipherable_custom_data = GetIniBool(
      L"Rendering", L"disassemble_undecipherable_custom_data", false, NULL);
  G->patch_cb_offsets =
      GetIniBool(L"Rendering", L"patch_assembly_cb_offsets", false, NULL);
  G->recursive_include =
      GetIniBoolOrInt(L"Rendering", L"recursive_include", false, NULL);
  ...
  // Automatic section
  G->decompiler_settings.fixSvPosition =
      GetIniBool(L"Rendering", L"fix_sv_position", false, NULL);
  G->decompiler_settings.recompileVs =
      GetIniBool(L"Rendering", L"recompile_all_vs", false, NULL);
  if (GetIniStringAndLog(L"Rendering", L"fix_ZRepair_DepthTexture1", 0, setting,
                         MAX_PATH)) {
    char buf[MAX_PATH];
  ...
  if (GetIniStringAndLog(L"Rendering", L"fix_ZRepair_DepthTexture2", 0, setting,
                         MAX_PATH)) {
    char buf[MAX_PATH];
    wcstombs(buf, setting, MAX_PATH);
    char *end = RightStripA(buf);
  ...
  if (GetIniStringAndLog(L"Rendering", L"fix_ZRepair_ZPosCalc1", 0, setting,
                         MAX_PATH))
    G->decompiler_settings.ZRepair_ZPosCalc1 = readStringParameter(setting);
  if (GetIniStringAndLog(L"Rendering", L"fix_ZRepair_ZPosCalc2", 0, setting,
                         MAX_PATH))
    G->decompiler_settings.ZRepair_ZPosCalc2 = readStringParameter(setting);
  if (GetIniStringAndLog(L"Rendering", L"fix_ZRepair_PositionTexture", 0,
                         setting, MAX_PATH))
    G->decompiler_settings.ZRepair_PositionTexture =
        readStringParameter(setting);
  if (GetIniStringAndLog(L"Rendering", L"fix_ZRepair_PositionCalc", 0, setting,
                         MAX_PATH))
    G->decompiler_settings.ZRepair_WorldPosCalc = readStringParameter(setting);
  if (GetIniStringAndLog(L"Rendering", L"fix_ZRepair_Dependencies1", 0, setting,
                         MAX_PATH)) {
    char buf[MAX_PATH];
    wcstombs(buf, setting, MAX_PATH);
    char *start = buf;
    ...
    while (*start) {
      ...
      while (*end != ',' && *end && *end != ' ')
        ++end;
      G->decompiler_settings.ZRepair_Dependencies1.push_back(
          string(start, end));
      start = end;
      if (*start == ',')
        ++start;
  ...
  if (GetIniStringAndLog(L"Rendering", L"fix_ZRepair_Dependencies2", 0, setting,
                         MAX_PATH)) {
    char buf[MAX_PATH];
    wcstombs(buf, setting, MAX_PATH);
    char *start = buf;
    ...
    while (*start) {
      ...
      while (*end != ',' && *end && *end != ' ')
        ++end;
      G->decompiler_settings.ZRepair_Dependencies2.push_back(
          string(start, end));
      start = end;
      if (*start == ',')
        ++start;
  ...
  if (GetIniStringAndLog(L"Rendering", L"fix_InvTransform", 0, setting,
                         MAX_PATH)) {
    char buf[MAX_PATH];
    wcstombs(buf, setting, MAX_PATH);
    char *start = buf;
  ...
  if (GetIniStringAndLog(L"Rendering", L"fix_ZRepair_DepthTextureHash", 0,
                         setting, MAX_PATH)) {
    uint32_t hash;
  ...
  if (GetIniStringAndLog(L"Rendering", L"fix_BackProjectionTransform1", 0,
                         setting, MAX_PATH))
    G->decompiler_settings.BackProject_Vector1 = readStringParameter(setting);
  if (GetIniStringAndLog(L"Rendering", L"fix_BackProjectionTransform2", 0,
                         setting, MAX_PATH))
    G->decompiler_settings.BackProject_Vector2 = readStringParameter(setting);
  if (GetIniStringAndLog(L"Rendering", L"fix_ObjectPosition1", 0, setting,
                         MAX_PATH))
    G->decompiler_settings.ObjectPos_ID1 = readStringParameter(setting);
  if (GetIniStringAndLog(L"Rendering", L"fix_ObjectPosition2", 0, setting,
                         MAX_PATH))
    G->decompiler_settings.ObjectPos_ID2 = readStringParameter(setting);
  if (GetIniStringAndLog(L"Rendering", L"fix_ObjectPosition1Multiplier", 0,
                         setting, MAX_PATH))
    G->decompiler_settings.ObjectPos_MUL1 = readStringParameter(setting);
  if (GetIniStringAndLog(L"Rendering", L"fix_ObjectPosition2Multiplier", 0,
                         setting, MAX_PATH))
    G->decompiler_settings.ObjectPos_MUL2 = readStringParameter(setting);
  if (GetIniStringAndLog(L"Rendering", L"fix_MatrixOperand1", 0, setting,
                         MAX_PATH))
    G->decompiler_settings.MatrixPos_ID1 = readStringParameter(setting);
  if (GetIniStringAndLog(L"Rendering", L"fix_MatrixOperand1Multiplier", 0,
                         setting, MAX_PATH))
  ...
  ParseCommandList(L"Present", &G->present_command_list,
                   &G->post_present_command_list, NULL);

  LogInfo("[ClearRenderTargetView]\n");
  G->clear_rtv_command_list.clear();
  G->post_clear_rtv_command_list.clear();
  ParseCommandList(L"ClearRenderTargetView", &G->clear_rtv_command_list,
                   &G->post_clear_rtv_command_list, NULL);

  LogInfo("[ClearDepthStencilView]\n");
  G->clear_dsv_command_list.clear();
  G->post_clear_dsv_command_list.clear();
  ParseCommandList(L"ClearDepthStencilView", &G->clear_dsv_command_list,
                   &G->post_clear_dsv_command_list, NULL);

  LogInfo("[ClearUnorderedAccessViewUint]\n");
  G->clear_uav_uint_command_list.clear();
  G->post_clear_uav_uint_command_list.clear();
  ParseCommandList(L"ClearUnorderedAccessViewUint",
                   &G->clear_uav_uint_command_list,
                   &G->post_clear_uav_uint_command_list, NULL);

  LogInfo("[ClearUnorderedAccessViewFloat]\n");
  G->clear_uav_float_command_list.clear();
  G->post_clear_uav_float_command_list.clear();
  ParseCommandList(L"ClearUnorderedAccessViewFloat",
                   &G->clear_uav_float_command_list,
                   &G->post_clear_uav_float_command_list, NULL);
...
void SavePersistentSettings() {
  ...
  fputs("; AUTOMATICALLY GENERATED FILE - DO NOT EDIT\n"
        ";\n"
        "; 3DMigoto will overwrite this file whenever any persistent settings "
        "are\n"
        "; altered by hot key or command list. Tag global variables with the "
        "\"persist\"\n"
        "; keyword to save them in this file. Use the post keyword in the "
        "[Constants]\n"
        "; command list if you need to do any intialisation after this file is "
        "loaded.\n"
        ";\n"
        "[Constants]\n",
        f);
...
void ReloadConfig(HackerDevice *device) {
  ...
  if (mHackerContext) {
    ...
    LogOverlay(LOG_DIRE,
               "BUG: No HackerContext at ReloadConfig - please report this\n");
  }
