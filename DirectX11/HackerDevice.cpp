...
// This gives us access to every D3D11 call for a device, and override the
// pieces needed.

// Object			OS				D3D11 version
// Feature level ID3D11Device
// Win7			11.0			11.0 ID3D11Device1	Platform
// update	11.1			11.1 ID3D11Device2
// Win8.1			11.2 ID3D11Device3
// Win10			11.3
// ID3D11Device4					11.4
...
HackerDevice *lookup_hacker_device(IUnknown *unknown) {
  ...
  if (!ret) {
    ...
    if (SUCCEEDED(
            unknown->QueryInterface(IID_IDXGIObject, (void **)&dxgi_obj))) {
      UINT size;
      if (SUCCEEDED(dxgi_obj->GetPrivateData(IID_HackerDevice, &size, &ret))) {
        LogInfo("Notice: Unwrapped device and COM Identity violation, Found "
                "HackerDevice via GetPrivateData strategy\n");
        ret->AddRef();
      }
      dxgi_obj->Release();
    }
...
static IUnknown *register_hacker_device(HackerDevice *hacker_device) {
  ...
  if (FAILED(hacker_device->GetPassThroughOrigDevice1()->QueryInterface(
  ...
  LogInfo(
      "register_hacker_device: Registering IUnknown: %p -> HackerDevice: %p\n",
...
static void unregister_hacker_device(HackerDevice *hacker_device) {
  ...
  if (i != device_map.end()) {
    if (i->second == hacker_device) {
      LogInfo("unregister_hacker_device: Unregistering IUnknown %p -> "
              "HackerDevice %p\n",
              real_unknown, hacker_device);
      device_map.erase(i);
    } else {
...
HackerDevice::HackerDevice(ID3D11Device1 *pDevice1,
    ...
    : mStereoHandle(0), mStereoResourceView(0), mStereoTexture(0),
      mIniResourceView(0), mIniTexture(0), mZBufferResourceView(0) {
...
HRESULT HackerDevice::CreateStereoParamResources() {
  ...
  if (nvret != NVAPI_OK) {
    mStereoHandle = 0;
    LogInfo("HackerDevice::CreateStereoParamResources "
            "NvAPI_Stereo_CreateHandleFromIUnknown failed: %d\n",
            nvret);
    return nvret;
  }
  ...
  // Since we need to bind the texture to a shader input, we also need a
  // resource view.
  ...
  hr = mOrigDevice1->CreateShaderResourceView(mStereoTexture, &descRV,
                                              &mStereoResourceView);
  if (FAILED(hr)) {
    LogInfo("    call failed with result = %x.\n", hr);
    return hr;
...
HRESULT HackerDevice::CreateIniParamResources() {
  ...
  if (G->iniParamsReserved > INI_PARAMS_SIZE_WARNING) {
    LogOverlay(LOG_NOTICE,
               "NOTICE: %d requested IniParams exceeds the recommended %d\n",
  ...
  // Stuff the constants read from the .ini file into the subresource data
  // structure, so we can init the texture with them.
  initialData.pSysMem = G->iniParams.data();
  initialData.SysMemPitch =
      sizeof(DirectX::XMFLOAT4) *
      (UINT)G->iniParams
          .size(); // Ignored for Texture1D, but still recommended for debugging
  ...
  desc.BindFlags =
      D3D11_BIND_SHADER_RESOURCE; // As resource view, access via t120
  ...
  // Since we need to bind the texture to a shader input, we also need a
  // resource view. The pDesc is set to NULL so that it will simply use the desc
  // format above.
  ...
  ret = mOrigDevice1->CreateShaderResourceView(mIniTexture, NULL,
                                               &mIniResourceView);
  if (FAILED(ret)) {
...
void HackerDevice::CreatePinkHuntingResources() {
  ...
  if (G->hunting &&
      (G->marking_mode == MarkingMode::PINK || G->config_reloadable)) {
    char *hlsl = "float4 pshader() : SV_Target0"
                 "{"
                 "	return float4(1,0,1,1);"
                 "}";

    ID3D10Blob *blob = NULL;
    HRESULT hr = D3DCompile(hlsl, strlen(hlsl), "JustPink", NULL, NULL,
                            "pshader", "ps_4_0", 0, 0, &blob, NULL);
    LogInfo("  Created pink mode pixel shader: %d\n", hr);
    if (SUCCEEDED(hr)) {
      hr = mOrigDevice1->CreatePixelShader((DWORD *)blob->GetBufferPointer(),
...
HRESULT HackerDevice::SetGlobalNVSurfaceCreationMode() {
  ...
  if (mStereoHandle && G->gSurfaceCreateMode >= 0) {
    ...
    hr = Profiling::NvAPI_Stereo_SetSurfaceCreationMode(
        mStereoHandle, (NVAPI_STEREO_SURFACECREATEMODE)G->gSurfaceCreateMode);
    if (hr != NVAPI_OK) {
...
// Save reference to corresponding HackerContext during CreateDevice, needed for
// GetImmediateContext.
...
HackerSwapChain *HackerDevice::GetHackerSwapChain() { return mHackerSwapChain; }
...
IUnknown *HackerDevice::GetIUnknown() { return mUnknown; }
...
// -----------------------------------------------------------------------------------------------
// ToDo: I'd really rather not have these standalone utilities here, this file
// should ideally be only HackerDevice and it's methods.  Because of our
// spaghetti Globals+Utils, it gets too involved to move these out right now.

// For any given vertex or pixel shader from the ShaderFixes folder, we need to
// track them at load time so that we can associate a given active shader with
// an override file.  This allows us to reload the shaders dynamically, and do
// on-the-fly fix testing. ShaderModel is usually something like "vs_5_0", but
// "bin" is a valid ShaderModel string, and tells the reloader to disassemble
// the .bin file to determine the shader model.

// Currently, critical lock must be taken BEFORE this is called.

static void RegisterForReload(ID3D11DeviceChild *ppShader, UINT64 hash,
                              wstring shaderType, string shaderModel,
                              ID3D11ClassLinkage *pClassLinkage,
                              ID3DBlob *byteCode, FILETIME timeStamp,
                              wstring text,
                              bool deferred_replacement_candidate) {
  LogInfo(
      "    shader registered for possible reloading: %016llx_%ls as %s - %ls\n",
  ...
  G->mReloadedShaders[ppShader].deferred_replacement_candidate =
      deferred_replacement_candidate;
  G->mReloadedShaders[ppShader].deferred_replacement_processed = false;
}

// Helper routines for ReplaceShader, as a way to factor out some of the inline
// code, in order to make it more clear, and as a first step toward full
// refactoring.

// This routine exports the original binary shader from the game, the cso.  It
// is a hidden feature in the d3dx.ini.  Seems like it might be nice to have
// them named *_orig.bin, to make them more clear.

static void ExportOrigBinary(UINT64 hash, const wchar_t *pShaderType,
  ...
  swprintf_s(path, MAX_PATH, L"%ls\\%016llx-%ls.bin", G->SHADER_CACHE_PATH,
             hash, pShaderType);
  ...
  if (f != INVALID_HANDLE_VALUE) {
    ...
    while (f != INVALID_HANDLE_VALUE) {
      ...
      CloseHandle(f);
      if (dataSize == pBytecodeLength &&
          !memcmp(pShaderBytecode, buf, dataSize))
        exists = true;
      delete[] buf;
      if (exists)
        break;
      swprintf_s(path, MAX_PATH, L"%ls\\%016llx-%ls_%d.bin",
                 G->SHADER_CACHE_PATH, hash, pShaderType, ++cnt);
      f = CreateFile(path, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING,
                     FILE_ATTRIBUTE_NORMAL, NULL);
    }
  }
...
static bool CheckCacheTimestamp(HANDLE binHandle, wchar_t *binPath,
                                FILETIME &pTimeStamp) {
  ...
  if (GetFileLastWriteTime(txtPath, &txtTime) &&
      GetFileTime(binHandle, NULL, NULL, &binTime)) {
...
static bool LoadCachedShader(wchar_t *binPath, const wchar_t *pShaderType,
                             __out char *&pCode, SIZE_T &pCodeSize,
                             string &pShaderModel, FILETIME &pTimeStamp) {
  HANDLE f;
  DWORD codeSize, readSize;

  f = CreateFile(binPath, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING,
                 FILE_ATTRIBUTE_NORMAL, NULL);
  if (f == INVALID_HANDLE_VALUE)
    return false;
  ...
  if (!ReadFile(f, pCode, codeSize, &readSize, 0) || codeSize != readSize) {
...
static bool LoadBinaryShaders(__in UINT64 hash, const wchar_t *pShaderType,
  ...
  swprintf_s(path, MAX_PATH, L"%ls\\%016llx-%ls_replace.bin", G->SHADER_PATH,
             hash, pShaderType);
  if (LoadCachedShader(path, pShaderType, pCode, pCodeSize, pShaderModel,
                       pTimeStamp))
    return true;

  // If we can't find an HLSL compiled version, look for ASM assembled one.
  swprintf_s(path, MAX_PATH, L"%ls\\%016llx-%ls.bin", G->SHADER_PATH, hash,
             pShaderType);
  return LoadCachedShader(path, pShaderType, pCode, pCodeSize, pShaderModel,
                          pTimeStamp);
}

// Load an HLSL text file as the replacement shader.  Recompile it using
// D3DCompile. If caching is enabled, save a .bin replacement for this new
// shader.

static bool ReplaceHLSLShader(__in UINT64 hash, const wchar_t *pShaderType,
                              __in const void *pShaderBytecode,
                              SIZE_T pBytecodeLength,
                              const char *pOverrideShaderModel,
                              __out char *&pCode, SIZE_T &pCodeSize,
                              string &pShaderModel, FILETIME &pTimeStamp,
                              wstring &pHeaderLine) {
  wchar_t path[MAX_PATH];
  ...
  swprintf_s(path, MAX_PATH, L"%ls\\%016llx-%ls_replace.txt", G->SHADER_PATH,
             hash, pShaderType);
  f = CreateFile(path, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING,
                 FILE_ATTRIBUTE_NORMAL, NULL);
  if (f != INVALID_HANDLE_VALUE) {
    ...
    if (!ReadFile(f, srcData, srcDataSize, &readSize, 0) ||
        !GetFileTime(f, NULL, NULL, &ftWrite) || srcDataSize != readSize)
      LogInfo("    Error reading file.\n");
    CloseHandle(f);
    ...
    if (shaderModel.empty()) {
      ...
      // Any HLSL compiled shaders are reloading candidates, if moved to
      // ShaderFixes
      ...
      HRESULT ret = D3DCompile(
          srcData, srcDataSize, apath, 0,
          G->recursive_include == -1 ? D3D_COMPILE_STANDARD_FILE_INCLUDE
                                     : &include_handler,
          "main", tmpShaderModel, D3DCOMPILE_OPTIMIZATION_LEVEL3, 0,
          &compiledOutput, &errorMsgs);
      delete[] srcData;
      srcData = 0;
      if (compiledOutput) {
      ...
      if (G->CACHE_SHADERS && pCode) {
        swprintf_s(path, MAX_PATH, L"%ls\\%016llx-%ls_replace.bin",
                   G->SHADER_PATH, hash, pShaderType);
        FILE *fw;
        wfopen_ensuring_access(&fw, path, L"wb");
        if (fw) {
...
// If a matching file exists, load an ASM text shader as a replacement for a
// shader. Reassemble it, and return the binary.
//
// Changing the output of this routine to be simply .bin files. We had some old
// test code for assembler validation, but that just causes confusion.  Retiring
// the *_reasm.txt files as redundant. Files are like:
//  cc79d4a79b16b59c-vs.txt  as ASM text
//  cc79d4a79b16b59c-vs.bin  as reassembled binary shader code
//
// Using this naming convention because we already have multiple fixes that use
// the *-vs.txt format to mean ASM text files, and changing all of those seems
// unnecessary.  This will parallel the use of HLSL files like:
...
static bool ReplaceASMShader(__in UINT64 hash, const wchar_t *pShaderType,
                             const void *pShaderBytecode,
                             SIZE_T pBytecodeLength, __out char *&pCode,
                             SIZE_T &pCodeSize, string &pShaderModel,
                             FILETIME &pTimeStamp, wstring &pHeaderLine) {
  wchar_t path[MAX_PATH];
  HANDLE f;
  string shaderModel;

  swprintf_s(path, MAX_PATH, L"%ls\\%016llx-%ls.txt", G->SHADER_PATH, hash,
             pShaderType);
  f = CreateFile(path, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING,
                 FILE_ATTRIBUTE_NORMAL, NULL);
  if (f != INVALID_HANDLE_VALUE) {
    LogInfo(
        "    Replacement ASM shader found. Assembling replacement ASM code.\n");
    WarnIfConflictingShaderExists(path, end_user_conflicting_shader_msg);
    ...
    if (shaderModel.empty()) {
      ...
      try {
        ...
        byteCode = AssembleFluganWithOptionalSignatureParsing(
            &asmTextBytes, G->assemble_signature_comments, &byteCode,
            &parse_errors);

        // Assuming the re-assembly worked, let's make it the active shader
        // code.
        ...
        if (parse_errors.empty()) {
          if (G->CACHE_SHADERS && pCode && parse_errors.empty()) {
            ...
            FILE *fw;
            swprintf_s(path, MAX_PATH, L"%ls\\%016llx-%ls.bin", G->SHADER_PATH,
                       hash, pShaderType);
            wfopen_ensuring_access(&fw, path, L"wb");
            if (fw) {
              ...
              // Set the last modified timestamp on the cached shader to match
              // the .txt file it is created from, so we can later check its
              // validity:
              set_file_last_write_time(path, &ftWrite);
            } else {
...
static bool DecompileAndPossiblyPatchShader(
    __in UINT64 hash, const wchar_t *pShaderType, const void *pShaderBytecode,
    SIZE_T BytecodeLength, __out char *&pCode, SIZE_T &pCodeSize,
    string &pShaderModel, FILETIME &pTimeStamp, wstring &pHeaderLine,
    const wchar_t *shaderType, string &foundShaderModel, FILETIME &timeStamp,
  ...
  if (!G->EXPORT_HLSL && !G->decompiler_settings.fixSvPosition &&
      !G->decompiler_settings.recompileVs)
    return NULL;

  // Skip?
  swprintf_s(val, MAX_PATH, L"%ls\\%016llx-%ls_bad.txt", G->SHADER_PATH, hash,
             shaderType);
  ...
  if (G->EXPORT_HLSL >= 1)
    swprintf_s(val, MAX_PATH, L"%ls\\%016llx-%ls_replace.txt",
               G->SHADER_CACHE_PATH, hash, shaderType);
  else
    swprintf_s(val, MAX_PATH, L"%ls\\%016llx-%ls_replace.txt", G->SHADER_PATH,
               hash, shaderType);

  // If we can open the file already, it exists, and thus we should skip doing
  // this slow operation again.
  if (GetFileAttributes(val) != INVALID_FILE_ATTRIBUTES)
    return NULL;
  ...
  const string decompiledCode =
      DecompileBinaryHLSL(p, patched, shaderModel, errorOccurred);
  ...
  if ((G->EXPORT_HLSL >= 1) || (G->EXPORT_FIXED && patched)) {
    ...
    // Now also write the ASM text to the shader file as a set of comments at
    // the bottom. That will make the ASM code the master reference for fixing
    // shaders, and should be more convenient, especially in light of the
    // numerous decompiler bugs we see.
    if (G->EXPORT_HLSL >= 2) {
      fprintf_s(fw, "\n\n/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Original ASM "
                    "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
  ...
  // Let's re-compile every time we create a new one, regardless.  Previously
  // this would only re-compile after auto-fixing shaders. This makes shader
  // Decompiler errors more obvious.
  ...
  hr = D3DCompile(decompiledCode.c_str(), decompiledCode.size(), apath, 0,
                  D3D_COMPILE_STANDARD_FILE_INCLUDE, "main", tmpShaderModel,
                  D3DCOMPILE_OPTIMIZATION_LEVEL3, 0, &pCompiledOutput,
                  &pErrorMsgs);
  ...
  if (LogFile && pErrorMsgs) {
    ...
    // And write the errors to the HLSL file as comments too, as a more
    // convenient spot to see them.
    fprintf_s(fw, "\n\n/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~ HLSL errors "
                  "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
  ...
  // If requested by .ini, also write the newly re-compiled assembly code to the
  // file.  This gives a direct comparison between original ASM, and recompiled
  // ASM.
  if ((G->EXPORT_HLSL >= 3) && pCompiledOutput) {
    asmText =
        BinaryToAsmText(pCompiledOutput->GetBufferPointer(),
                        pCompiledOutput->GetBufferSize(), G->patch_cb_offsets);
    if (asmText.empty()) {
...
// This is called at launch to replace any shaders that we might want patched to
// fix problems. It would previously use both ShaderCache, and ShaderFixes both
// to fix shaders, but this is problematic in that broken shaders dumped as part
// of universal cache could be buggy, and generated visual anomolies.  Moreover,
// we don't really want every file to patched, just the ones we need.

// I'm moving to a model where only stuff in ShaderFixes is active, and stuff in
// ShaderCache is for reference. This will allow us to dump and use the
// ShaderCache for offline fixes, looking for similar fix patterns, and also
// make them live by moving them to ShaderFixes. For auto-fixed shaders- rather
// than leave them in ShaderCache, when they are fixed, we'll move them into
...
//
// In general I (bo3b) agree, but would hesitate to apply a C style guide to
// kinda/sorta C++ code.  A sort of mix of the linux guide and C++ is Google's
// Style Guide:
//   https://google.github.io/styleguide/cppguide.html
// Apparently serious C++ programmers hate it, so that must mean it makes things
// simpler and clearer. We could stand to even have just a style template in VS
// that would make everything consistent at a minimum.  I'd say refactoring this
// sucker is higher value though.
//
// I hate to make a bad thing worse, but I need to return yet another parameter
// here, the string read from the first line of the HLSL file.  This the logical
// place for it because the file is already open and read into memory.

char *HackerDevice::_ReplaceShaderFromShaderFixes(
    UINT64 hash, const wchar_t *shaderType, const void *pShaderBytecode,
  ...
  if (G->EXPORT_SHADERS)
    CreateAsmTextFile(G->SHADER_CACHE_PATH, hash, shaderType, pShaderBytecode,
                      BytecodeLength, G->patch_cb_offsets);

  // Read the binary compiled shaders, as previously cached shaders.  This is
  // how fixes normally ship, so that we just load previously compiled/assembled
  // shaders.
  if (LoadBinaryShaders(hash, shaderType, pCode, pCodeSize, foundShaderModel,
                        timeStamp))
    return pCode;

  // Load previously created HLSL shaders, but only from ShaderFixes.
  if (ReplaceHLSLShader(hash, shaderType, pShaderBytecode, BytecodeLength,
                        overrideShaderModel, pCode, pCodeSize, foundShaderModel,
                        timeStamp, headerLine)) {
    return pCode;
  }

  // If still not found, look for replacement ASM text shaders.
  if (ReplaceASMShader(hash, shaderType, pShaderBytecode, BytecodeLength, pCode,
                       pCodeSize, foundShaderModel, timeStamp, headerLine)) {
    return pCode;
  }

  if (DecompileAndPossiblyPatchShader(
          hash, shaderType, pShaderBytecode, BytecodeLength, pCode, pCodeSize,
...
template <class ID3D11Shader,
          HRESULT (__stdcall ID3D11Device::*OrigCreateShader)(
              THIS_ __in const void *pShaderBytecode,
              __in SIZE_T BytecodeLength,
              __in_opt ID3D11ClassLinkage *pClassLinkage,
              __out_opt ID3D11Shader **ppShader)>
HRESULT HackerDevice::ReplaceShaderFromShaderFixes(
    UINT64 hash, const void *pShaderBytecode, SIZE_T BytecodeLength,
  ...
  char *replaceShader = _ReplaceShaderFromShaderFixes(
      hash, shaderType, pShaderBytecode, BytecodeLength, replaceShaderSize,
  ...
  *ppShader = NULL; // Appease the static analysis gods
  hr = (mOrigDevice1->*OrigCreateShader)(replaceShader, replaceShaderSize,
                                         pClassLinkage, ppShader);
  if (FAILED(hr)) {
  ...
  if (G->hunting) {
    ...
    if (SUCCEEDED(hr)) {
      ...
      EnterCriticalSectionPretty(&G->mCriticalSection);
      RegisterForReload(*ppShader, hash, shaderType, shaderModel, pClassLinkage,
                        blob, ftWrite, headerLine, false);
      LeaveCriticalSection(&G->mCriticalSection);
    }
  }

  // FIXME: We have some very similar data structures that we should merge
  // together: mReloadedShaders and mOriginalShader.
  KeepOriginalShader<ID3D11Shader, OrigCreateShader>(
      hash, shaderType, *ppShader, pShaderBytecode, BytecodeLength,
      pClassLinkage);
...
HRESULT HackerDevice::ProcessShaderNotFoundInShaderFixes(
  ...
  *ppShader = NULL; // Appease the static analysis gods
  hr = (mOrigDevice1->*OrigCreateShader)(pShaderBytecode, BytecodeLength,
                                         pClassLinkage, ppShader);
  if (FAILED(hr))
    return hr;
  ...
  // When in hunting mode, make a copy of the original binary, regardless.  This
  // can be replaced, but we'll at least have a copy for every shader seen. If
  // we are performing any sort of deferred shader replacement, such as pipline
  // state analysis we always need to keep a copy of the original bytecode for
  // later analysis. For now the shader regex engine counts as deferred, though
  // that may change with optimisations in the future.
  if (G->hunting || !shader_regex_groups.empty()) {
    ...
    if (SUCCEEDED(hr)) {
      memcpy(blob->GetBufferPointer(), pShaderBytecode, blob->GetBufferSize());
      RegisterForReload(*ppShader, hash, shaderType, "bin", pClassLinkage, blob,
                        {0}, L"", true);
...
bool HackerDevice::NeedOriginalShader(UINT64 hash) {
  ...
  if (G->hunting && (G->marking_mode == MarkingMode::ORIGINAL ||
                     G->config_reloadable || G->show_original_enabled))
    return true;
...
// If called with IDXGIDevice, that's the game trying to access the original
// DXGIFactory to get access to the swap chain.  We need to return a
// HackerDXGIDevice so that we can get access to that swap chain.
//
// This is the 'secret' path to getting the DXGIFactory and thus the swap chain,
// without having to go direct to DXGI calls. As described:
...
//
// Next up, it seems that we also need to handle a QueryInterface(IDXGIDevice1),
// as WatchDogs uses that call.  Another oddity: this device is called to return
// the same device. ID3D11Device->QueryInterface(ID3D11Device).  No idea why,
// but we need to return our wrapped version.
//
// 1-4-18: No longer using this technique, we have a direct hook on
// CreateSwapChain, which will catch all variants. But leaving documentation for
// awhile.

// New addition, we need to also look for QueryInterface casts to different
// types. In Dragon Age, it seems clear that they are upcasting their
// ID3D11Device to an ID3D11Device1, and if we don't wrap that, we have an
// object leak where they can bypass us.
//
// Initial call needs to be LogDebug, because this is otherwise far to chatty in
// the log.  That can be kind of misleading, so careful with missing log info.
// To keep it consistent, all normal cases will be LogDebug, error states are
// LogInfo.
...
HRESULT STDMETHODCALLTYPE HackerDevice::QueryInterface(
  ...
  // No need for further checks of null ppvObject, as it could not have
  // successfully called the original in that case.
  ...
  if (riid == __uuidof(ID3D11Device)) {
    ...
    LogDebug("  return HackerDevice(%s@%p) wrapper of %p\n", type_name(this),
             this, mRealOrigDevice1);
  } else if (riid == __uuidof(ID3D11Device1)) {
    // Well, bizarrely, this approach to upcasting to a ID3D11Device1 is
    // supported on Win7, but only if you have the 'evil update', the platform
    // update installed.  Since that is an optional update, that certainly means
    // that numerous people do not have it installed. Ergo, a game developer
    // cannot in good faith just assume that it's there, and it's very unlikely
    // they would require it. No performance advantage on Win8. So, that means
    // that a game developer must support a fallback path, even if they actually
    // want Device1 for some reason.
    //
    // Sooo... Current plan is to return an error here, and pretend that the
    // platform update is not installed, or missing feature on Win8.1.  This
    // will force the game to use a more compatible path and make our job
    // easier. This worked in DragonAge, to avoid a crash. Wrapping Device1 also
    // progressed but adds a ton of undesirable complexity, so let's keep it
    // simpler since we don't seem to lose anything. Not features, not
    // performance.
    //
    ...
    if (!G->enable_platform_update) {
      LogInfo("  returns E_NOINTERFACE as error for ID3D11Device1 (try "
              "allow_platform_update=1 if the game refuses to run).\n");
      *ppvObject = NULL;
  ...
  // Warn if returning unwrapped higher device interfaces
  // Note: Only ID3D11Device2 is checked here as Device3-5 require newer SDK
  if (riid == __uuidof(ID3D11Device2)) {
    LogInfo("  WARNING: Returning unwrapped %s - wrapper consistency may be "
            "affected\n",
...
// These are the boilerplate routines that are necessary to pass through any
// calls to these to Direct3D.  Since Direct3D does not have proper objects, we
// can't rely on super class calls.

STDMETHODIMP HackerDevice::CreateUnorderedAccessView(
    THIS_
        /* [annotation] */
...
STDMETHODIMP HackerDevice::CreateRenderTargetView(
    THIS_
        /* [annotation] */
        __in ID3D11Resource *pResource,
    /* [annotation] */
...
STDMETHODIMP HackerDevice::CreateDepthStencilView(
    THIS_
        /* [annotation] */
        __in ID3D11Resource *pResource,
    /* [annotation] */
  ...
  LogDebug("HackerDevice::CreateDepthStencilView(%s@%p)\n", type_name(this),
           this);
  return mOrigDevice1->CreateDepthStencilView(pResource, pDesc,
                                              ppDepthStencilView);
}

STDMETHODIMP HackerDevice::CreateInputLayout(
    THIS_
        /* [annotation] */
  ...
  ret = mOrigDevice1->CreateInputLayout(pInputElementDescs, NumElements,
  ...
  if (G->hunting && SUCCEEDED(ret) && ppInputLayout && *ppInputLayout) {
    ...
    if (SUCCEEDED(D3DCreateBlob(sizeof(D3D11_INPUT_ELEMENT_DESC) * NumElements,
                                &blob))) {
      memcpy(blob->GetBufferPointer(), pInputElementDescs,
             blob->GetBufferSize());
...
STDMETHODIMP
HackerDevice::CreateClassLinkage(THIS_
                                     /* [annotation] */
...
STDMETHODIMP
HackerDevice::CreateBlendState(THIS_
                                   /* [annotation] */
...
STDMETHODIMP HackerDevice::CreateDepthStencilState(
    THIS_
        /* [annotation] */
        __in const D3D11_DEPTH_STENCIL_DESC *pDepthStencilDesc,
    /* [annotation] */
    __out_opt ID3D11DepthStencilState **ppDepthStencilState) {
  return mOrigDevice1->CreateDepthStencilState(pDepthStencilDesc,
                                               ppDepthStencilState);
}

STDMETHODIMP HackerDevice::CreateSamplerState(
    THIS_
        /* [annotation] */
...
STDMETHODIMP
HackerDevice::CreateQuery(THIS_
                              /* [annotation] */
...
STDMETHODIMP
HackerDevice::CreatePredicate(THIS_
                                  /* [annotation] */
...
STDMETHODIMP
HackerDevice::CreateCounter(THIS_
                                /* [annotation] */
...
STDMETHODIMP HackerDevice::OpenSharedResource(THIS_
  ...
  return mOrigDevice1->OpenSharedResource(hResource, ReturnedInterface,
                                          ppResource);
}
...
STDMETHODIMP
HackerDevice::CheckMultisampleQualityLevels(THIS_
                                                /* [annotation] */
                                                __in DXGI_FORMAT Format,
                                            /* [annotation] */
  ...
  return mOrigDevice1->CheckMultisampleQualityLevels(Format, SampleCount,
                                                     pNumQualityLevels);
}
...
STDMETHODIMP
HackerDevice::CheckCounter(THIS_
                               /* [annotation] */
  ...
  return mOrigDevice1->CheckCounter(pDesc, pType, pActiveCounters, szName,
                                    pNameLength, szUnits, pUnitsLength,
                                    szDescription, pDescriptionLength);
}

STDMETHODIMP HackerDevice::CheckFeatureSupport(
    THIS_ D3D11_FEATURE Feature,
    /* [annotation] */
    __out_bcount(FeatureSupportDataSize) void *pFeatureSupportData,
    UINT FeatureSupportDataSize) {
  return mOrigDevice1->CheckFeatureSupport(Feature, pFeatureSupportData,
                                           FeatureSupportDataSize);
}

STDMETHODIMP
HackerDevice::GetPrivateData(THIS_
                                 /* [annotation] */
                                 __in REFGUID guid,
                             /* [annotation] */
...
STDMETHODIMP
HackerDevice::SetPrivateDataInterface(THIS_
                                          /* [annotation] */
                                          __in REFGUID guid,
                                      /* [annotation] */
...
static bool check_texture_override_iteration(TextureOverride *textureOverride) {
  ...
  int currentIteration = textureOverride->iterations[0] =
      textureOverride->iterations[0] + 1;
...
template <typename DescType> static bool is_square_surface(DescType *desc) {
  return false;
}
...
static void
override_resource_desc_common_2d_3d(DescType *desc,
...
static void override_resource_desc(D3D11_TEXTURE3D_DESC *desc,
                                   TextureOverride *textureOverride) {
  override_resource_desc_common_2d_3d(desc, textureOverride);
}

template <typename DescType>
static const DescType *
process_texture_override(uint32_t hash, StereoHandle mStereoHandle,
                         const DescType *origDesc, DescType *newDesc,
  ...
  *oldMode = (NVAPI_STEREO_SURFACECREATEMODE)-1;
  ...
  if (newMode != (NVAPI_STEREO_SURFACECREATEMODE)-1) {
    ...
    if (NVAPI_OK !=
        Profiling::NvAPI_Stereo_SetSurfaceCreationMode(mStereoHandle, newMode))
      LogInfo("      call failed.\n");
  }
...
static void
restore_old_surface_create_mode(NVAPI_STEREO_SURFACECREATEMODE oldMode,
                                StereoHandle mStereoHandle) {
  if (oldMode != (NVAPI_STEREO_SURFACECREATEMODE)-1) {
    if (NVAPI_OK !=
        Profiling::NvAPI_Stereo_SetSurfaceCreationMode(mStereoHandle, oldMode))
...
STDMETHODIMP
HackerDevice::CreateBuffer(THIS_
                               /* [annotation] */
...
STDMETHODIMP HackerDevice::CreateTexture1D(
    THIS_
        /* [annotation] */
  ...
  HRESULT hr =
      mOrigDevice1->CreateTexture1D(pNewDesc, pInitialData, ppTexture1D);
...
static bool heuristic_could_be_possible_resolution(unsigned width,
                                                   unsigned height) {
...
STDMETHODIMP HackerDevice::CreateTexture2D(
    THIS_
        /* [annotation] */
  ...
  if (pInitialData && pInitialData->pSysMem) {
    LogDebugNoNL(
        "  pInitialData = %p->%p, SysMemPitch: %u, SysMemSlicePitch: %u ",
        pInitialData, pInitialData->pSysMem, pInitialData->SysMemPitch,
  ...
  // Actual creation:
  HRESULT hr =
      mOrigDevice1->CreateTexture2D(pNewDesc, pInitialData, ppTexture2D);
  restore_old_surface_create_mode(oldMode, mStereoHandle);
  if (ppTexture2D)
...
STDMETHODIMP
HackerDevice::CreateTexture3D(THIS_
                                  /* [annotation] */
  ...
  // Initial data is optional, so we might have zero data to add to the hash
  // there.
  uint32_t data_hash, hash;
  ...
  HRESULT hr =
      mOrigDevice1->CreateTexture3D(pNewDesc, pInitialData, ppTexture3D);
...
STDMETHODIMP HackerDevice::CreateShaderResourceView(
    THIS_
        /* [annotation] */
        __in ID3D11Resource *pResource,
    /* [annotation] */
  ...
  HRESULT hr =
      mOrigDevice1->CreateShaderResourceView(pResource, pDesc, ppSRView);

  // Check for depth buffer view.
  if (hr == S_OK && G->ZBufferHashToInject && ppSRView) {
    EnterCriticalSectionPretty(&G->mResourcesLock);
    unordered_map<ID3D11Resource *, ResourceHandleInfo>::iterator i =
        lookup_resource_handle_info(pResource);
...
static uint32_t hash_shader_bytecode(struct dxbc_header *header,
                                     SIZE_T BytecodeLength) {
  ...
  if (BytecodeLength <
      sizeof(struct dxbc_header) + header->num_sections * sizeof(uint32_t))
    return 0;

  for (i = 0; i < header->num_sections; i++) {
    section = (struct section_header *)((char *)header + offsets[i]);
    if (BytecodeLength < (char *)section - (char *)header +
                             sizeof(struct section_header) + section->size)
      return 0;

    for (j = 0; j < ARRAYSIZE(hash_whitelisted_sections); j++) {
      if (!strncmp(section->signature, hash_whitelisted_sections[j], 4))
        hash = crc32c_hw(hash, (char *)section + sizeof(struct section_header),
                         section->size);
    }
  }
...
template <class ID3D11Shader,
          HRESULT (__stdcall ID3D11Device::*OrigCreateShader)(
              ...
              __out_opt ID3D11Shader **ppShader)>
STDMETHODIMP
HackerDevice::CreateShader(THIS_
                               /* [annotation] */
                               __in const void *pShaderBytecode,
                           /* [annotation] */
                           __in SIZE_T BytecodeLength,
  ...
  if (!ppShader || !pShaderBytecode) {
    ...
    return (mOrigDevice1->*OrigCreateShader)(pShaderBytecode, BytecodeLength,
                                             pClassLinkage, ppShader);
  }
  ...
  hr = ReplaceShaderFromShaderFixes<ID3D11Shader, OrigCreateShader>(
      hash, pShaderBytecode, BytecodeLength, pClassLinkage, ppShader,
      shaderType);
...
STDMETHODIMP HackerDevice::CreateVertexShader(
    THIS_
        /* [annotation] */
        __in const void *pShaderBytecode,
    /* [annotation] */
  ...
  LogInfo("HackerDevice::CreateVertexShader called with BytecodeLength = %Iu, "
          "handle = %p, ClassLinkage = %p\n",
...
STDMETHODIMP HackerDevice::CreateGeometryShader(
    THIS_
        /* [annotation] */
        __in const void *pShaderBytecode,
    /* [annotation] */
  ...
  LogInfo("HackerDevice::CreateGeometryShader called with BytecodeLength = "
          "%Iu, handle = %p\n",
...
STDMETHODIMP HackerDevice::CreateGeometryShaderWithStreamOutput(
  ...
  HRESULT hr = mOrigDevice1->CreateGeometryShaderWithStreamOutput(
      pShaderBytecode, BytecodeLength, pSODeclaration, NumEntries,
      pBufferStrides, NumStrides, RasterizedStream, pClassLinkage,
      ppGeometryShader);
  LogInfo("  returns result = %x, handle = %p\n", hr,
          (ppGeometryShader ? *ppGeometryShader : NULL));

  return hr;
}

STDMETHODIMP
HackerDevice::CreatePixelShader(THIS_
                                    /* [annotation] */
                                    __in const void *pShaderBytecode,
                                /* [annotation] */
                                __in SIZE_T BytecodeLength,
  ...
  LogInfo("HackerDevice::CreatePixelShader called with BytecodeLength = %Iu, "
          "handle = %p, ClassLinkage = %p\n",
          BytecodeLength, pShaderBytecode, pClassLinkage);
...
STDMETHODIMP
HackerDevice::CreateHullShader(THIS_
                                   /* [annotation] */
                                   __in const void *pShaderBytecode,
                               /* [annotation] */
                               __in SIZE_T BytecodeLength,
  ...
  LogInfo("HackerDevice::CreateHullShader called with BytecodeLength = %Iu, "
          "handle = %p\n",
          BytecodeLength, pShaderBytecode);
...
STDMETHODIMP HackerDevice::CreateDomainShader(
    THIS_
        /* [annotation] */
        __in const void *pShaderBytecode,
    /* [annotation] */
  ...
  LogInfo("HackerDevice::CreateDomainShader called with BytecodeLength = %Iu, "
          "handle = %p\n",
          BytecodeLength, pShaderBytecode);
...
STDMETHODIMP HackerDevice::CreateComputeShader(
    THIS_
        /* [annotation] */
        __in const void *pShaderBytecode,
    /* [annotation] */
  ...
  LogInfo("HackerDevice::CreateComputeShader called with BytecodeLength = %Iu, "
          "handle = %p\n",
          BytecodeLength, pShaderBytecode);
...
STDMETHODIMP HackerDevice::CreateRasterizerState(
    THIS_
        /* [annotation] */
  ...
  if (pRasterizerDesc)
    LogDebug("HackerDevice::CreateRasterizerState called with\n"
             "  FillMode = %d, CullMode = %d, DepthBias = %d, DepthBiasClamp = "
             "%f, SlopeScaledDepthBias = %f,\n"
             "  DepthClipEnable = %d, ScissorEnable = %d, MultisampleEnable = "
             "%d, AntialiasedLineEnable = %d\n",
             pRasterizerDesc->FillMode, pRasterizerDesc->CullMode,
             pRasterizerDesc->DepthBias, pRasterizerDesc->DepthBiasClamp,
             pRasterizerDesc->SlopeScaledDepthBias,
             pRasterizerDesc->DepthClipEnable, pRasterizerDesc->ScissorEnable,
...
STDMETHODIMP HackerDevice::CreateDeferredContext(
    THIS_ UINT ContextFlags,
    /* [annotation] */
  ...
  HRESULT hr =
      mOrigDevice1->CreateDeferredContext(ContextFlags, ppDeferredContext);
  if (FAILED(hr)) {
  ...
  if (ppDeferredContext) {
    ...
    HRESULT res =
        (*ppDeferredContext)->QueryInterface(IID_PPV_ARGS(&origContext1));
    if (FAILED(res))
      origContext1 = static_cast<ID3D11DeviceContext1 *>(*ppDeferredContext);
    HackerContext *hackerContext =
        HackerContextFactory(mRealOrigDevice1, origContext1);
...
// "The GetImmediateContext method increments the reference count of the
// immediate context by one. "
//
// Fairly common to see this called all the time, so switching to LogDebug for
// this as a way to trim down normal log.

STDMETHODIMP_(void)
HackerDevice::GetImmediateContext(
    THIS_
        /* [annotation] */
  ...
  // We still need to call the original function to make sure the reference
  // counts are correct:
  ...
  if (mHackerContext == nullptr) {
    ...
    ID3D11DeviceContext1 *origContext1;
    HRESULT res =
        (*ppImmediateContext)->QueryInterface(IID_PPV_ARGS(&origContext1));
    if (FAILED(res))
    ...
    LogInfo("  HackerContext %p created to wrap %p\n", mHackerContext,
            *ppImmediateContext);
  } else if (mHackerContext->GetPossiblyHookedOrigContext1() !=
             *ppImmediateContext) {
    LogInfo("WARNING: mHackerContext %p found to be wrapping %p instead of %p "
            "at HackerDevice::GetImmediateContext!\n",
            mHackerContext, mHackerContext->GetPossiblyHookedOrigContext1(),
            *ppImmediateContext);
  }
...
HackerDevice::GetImmediateContext1(
  ...
  if (mHackerContext == nullptr) {
    LogInfo(
        "*** HackerContext1 missing at HackerDevice::GetImmediateContext1\n");
    ...
    LogInfo("  mHackerContext %p created to wrap %p\n", mHackerContext,
            *ppImmediateContext);
             ...
             *ppImmediateContext) {
    LogInfo("WARNING: mHackerContext %p found to be wrapping %p instead of %p "
            "at HackerDevice::GetImmediateContext1!\n",
            mHackerContext, mHackerContext->GetPossiblyHookedOrigContext1(),
            *ppImmediateContext);
  }

  *ppImmediateContext =
      reinterpret_cast<ID3D11DeviceContext1 *>(mHackerContext);
...
STDMETHODIMP HackerDevice::CreateDeferredContext1(
  ...
  LogInfo("HackerDevice::CreateDeferredContext1(%s@%p) called with flags = "
          "%#x, ptr:%p\n",
          type_name(this), this, ContextFlags, ppDeferredContext);

  HRESULT hr =
      mOrigDevice1->CreateDeferredContext1(ContextFlags, ppDeferredContext);
  if (FAILED(hr)) {
    LogInfo("  failed result = %x for %p\n", hr, ppDeferredContext);
    return hr;
  }
  ...
  if (ppDeferredContext) {
    analyse_iunknown(*ppDeferredContext);
    HackerContext *hackerContext =
        HackerContextFactory(mRealOrigDevice1, *ppDeferredContext);
    hackerContext->SetHackerDevice(this);
    hackerContext->Bind3DMigotoResources();
...
STDMETHODIMP HackerDevice::CreateRasterizerState1(
  ...
  return mOrigDevice1->CreateRasterizerState1(pRasterizerDesc,
                                              ppRasterizerState);
}
...
STDMETHODIMP HackerDevice::CreateDeviceContextState(
  ...
  return mOrigDevice1->CreateDeviceContextState(
...
STDMETHODIMP HackerDevice::OpenSharedResource1(
  ...
  return mOrigDevice1->OpenSharedResource1(hResource, returnedInterface,
                                           ppResource);
}
...
STDMETHODIMP HackerDevice::OpenSharedResourceByName(
    ...
    /* [annotation] */
    _Out_ void **ppResource) {
  return mOrigDevice1->OpenSharedResourceByName(lpName, dwDesiredAccess,
