...
// Strategy: This OriginalShaderInfo record and associated map is to allow us to
// keep track of every
//	pixelshader and vertexshader that are compiled from hlsl text from the
//ShaderFixes 	folder.  This keeps track of the original shader information using
//the ID3D11VertexShader* 	or ID3D11PixelShader* as a master key to the key map.
//	We are using the base class of ID3D11DeviceChild* since both descend
//from that, and that allows 	us to use the same structure for Pixel and Vertex
//shaders both.

// Info saved about originally overridden shaders passed in by the game in
// CreateVertexShader or CreatePixelShader that have been loaded as HLSL
//	shaderType is "vs" or "ps" or maybe later "gs" (type wstring for file
//name use) 	shaderModel is only filled in when a shader is replaced.  (type
//string for old D3 API use) 	linkage is passed as a parameter, seems to be
//rarely if ever used. 	byteCode is the original shader byte code passed in by
//game, or recompiled by override. 	timeStamp allows reloading/recompiling only
//modified shaders 	replacement is either ID3D11VertexShader or ID3D11PixelShader
...
// Key is the overridden shader that was given back to the game at
// CreateVertexShader (vs or ps)
typedef std::unordered_map<ID3D11DeviceChild *, OriginalShaderInfo>
    ShaderReloadMap;

// TODO: We can probably merge this into ShaderReloadMap
typedef std::unordered_map<ID3D11DeviceChild *, ID3D11DeviceChild *>
...
enum class FrameAnalysisOptions {
  ...
  // Masks:
  DUMP_XB_MASK =
      0x00000038, // CB+VB+IB, to check if a user specified any of these
...
static EnumName_t<wchar_t *, FrameAnalysisOptions> FrameAnalysisOptionNames[] =
    {
        ...
        {L"log",
         FrameAnalysisOptions::DEPRECATED}, // Left in the list for backwards
                                            // compatibility, but this is now
                                            // always enabled
        ...
        {L"dump_depth_jps",
         FrameAnalysisOptions::DUMP_DEPTH_JPS}, // Doesn't work yet
...
struct ShaderOverride {
  ...
  ShaderOverride()
      : depth_filter(DepthBufferFilter::NONE), partner_hash(0),
        allow_duplicate_hashes(1), filter_index(FLT_MAX),
...
static inline bool operator<(const ResourceSnapshot &lhs,
...
struct ShaderInfoData {
  // All are std::map or std::set so that ShaderUsage.txt is sorted - lookup
  // time is O(log N)
...
struct Globals {
  ...
  // SwapChain passthrough settings for modern games using flip-model
  bool allow_swapchain2_passthrough;
  bool allow_swapchain3_passthrough;
  bool allow_swapchain4_passthrough;

  int GAME_INTERNAL_WIDTH; // this variable stores the resolution width provided
                           // by the game (required for the upscaling feature)
  int GAME_INTERNAL_HEIGHT; // this variable stores the resolution height
                            // provided by the game (required for the upscaling
                            // feature)
  int SCREEN_WIDTH;
  int SCREEN_HEIGHT;
  int SCREEN_REFRESH;
  ...
  int EXPORT_HLSL; // 0=off, 1=HLSL only, 2=HLSL+OriginalASM, 3=
                   // HLSL+OriginalASM+recompiledASM
  bool EXPORT_SHADERS, EXPORT_FIXED, EXPORT_BINARY, CACHE_SHADERS,
      SCISSOR_DISABLE;
  ...
  std::set<uint32_t> mVisitedIndexBuffers; // std::set is sorted for consistent
                                           // order while hunting
  uint32_t mSelectedIndexBuffer;
  int mSelectedIndexBufferPos;
  std::set<UINT64>
      mSelectedIndexBuffer_VertexShader; // std::set so that shaders used with
                                         // an index buffer will be sorted in
                                         // log when marked
  std::set<UINT64>
      mSelectedIndexBuffer_PixelShader; // std::set so that shaders used with an
                                        // index buffer will be sorted in log
                                        // when marked

  std::set<uint32_t> mVisitedVertexBuffers; // std::set is sorted for consistent
                                            // order while hunting
  uint32_t mSelectedVertexBuffer;
  int mSelectedVertexBufferPos;
  std::set<UINT64>
      mSelectedVertexBuffer_VertexShader; // std::set so that shaders used with
                                          // an index buffer will be sorted in
                                          // log when marked
  std::set<UINT64>
      mSelectedVertexBuffer_PixelShader; // std::set so that shaders used with
                                         // an index buffer will be sorted in
                                         // log when marked

  std::set<UINT64>
      mVisitedVertexShaders;    // Only shaders seen since last hunting timeout;
                                // std::set for consistent order while hunting
  UINT64 mSelectedVertexShader; // Hash.  -1 now for unselected state. The
                                // shader selected using Input object.
  int mSelectedVertexShaderPos; // -1 for unselected state.
  std::set<uint32_t>
      mSelectedVertexShader_IndexBuffer; // std::set so that index buffers used
                                         // with a shader will be sorted in log
                                         // when marked
  std::set<uint32_t>
      mSelectedVertexShader_VertexBuffer; // std::set so that index buffers used
                                          // with a shader will be sorted in log
                                          // when marked

  std::set<UINT64> mVisitedPixelShaders; // std::set is sorted for consistent
                                         // order while hunting
  UINT64 mSelectedPixelShader;           // Hash.  -1 now for unselected state.
  int mSelectedPixelShaderPos;           // -1 for unselected state.
  std::set<uint32_t>
      mSelectedPixelShader_IndexBuffer; // std::set so that index buffers used
                                        // with a shader will be sorted in log
                                        // when marked
  std::set<uint32_t>
      mSelectedPixelShader_VertexBuffer; // std::set so that index buffers used
                                         // with a shader will be sorted in log
                                         // when marked
  ID3D11PixelShader *mPinkingShader;     // Special pixels shader to mark a
                                         // selection with hot pink.

  ShaderMap mShaders; // All shaders ever registered with CreateXXXShader
  ShaderReloadMap
      mReloadedShaders; // Shaders that were reloaded live from ShaderFixes
  ShaderReplacementMap
      mOriginalShaders; // When MarkingMode=Original, switch to original. Also
                        // used for show_original and shader reversion
  ...
  std::set<uint32_t> mRenderTargetInfo; // std::set so that ShaderUsage.txt is
                                        // sorted - lookup time is O(log N)
  std::set<uint32_t>
      mUnorderedAccessInfo; // std::set so that ShaderUsage.txt is sorted -
                            // lookup time is O(log N)
  std::set<uint32_t> mDepthTargetInfo;    // std::set so that ShaderUsage.txt is
                                          // sorted - lookup time is O(log N)
  std::set<uint32_t> mShaderResourceInfo; // std::set so that ShaderUsage.txt is
                                          // sorted - lookup time is O(log N)
  std::set<uint32_t> mCopiedResourceInfo; // std::set so that ShaderUsage.txt is
                                          // sorted - lookup time is O(log N)

  std::set<ID3D11Resource *>
      mVisitedRenderTargets; // std::set is sorted for consistent order while
                             // hunting
  ...
  std::set<ID3D11Resource *>
      mSelectedRenderTargetSnapshotList; // std::set so that render targets will
                                         // be sorted in log when marked
  // Relations
  std::map<UINT64, ShaderInfoData>
      mVertexShaderInfo; // std::map so that ShaderUsage.txt is sorted - lookup
                         // time is O(log N)
  std::map<UINT64, ShaderInfoData>
      mHullShaderInfo; // std::map so that ShaderUsage.txt is sorted - lookup
                       // time is O(log N)
  std::map<UINT64, ShaderInfoData>
      mDomainShaderInfo; // std::map so that ShaderUsage.txt is sorted - lookup
                         // time is O(log N)
  std::map<UINT64, ShaderInfoData>
      mGeometryShaderInfo; // std::map so that ShaderUsage.txt is sorted -
                           // lookup time is O(log N)
  std::map<UINT64, ShaderInfoData>
      mPixelShaderInfo; // std::map so that ShaderUsage.txt is sorted - lookup
                        // time is O(log N)
  std::map<UINT64, ShaderInfoData>
      mComputeShaderInfo; // std::map so that ShaderUsage.txt is sorted - lookup
                          // time is O(log N)

  Globals()
      :

        mSelectedRenderTargetSnapshot(0), mSelectedRenderTargetPos(-1),
        mSelectedRenderTarget((ID3D11Resource *)-1), mSelectedPixelShader(-1),
        mSelectedPixelShaderPos(-1), mSelectedVertexShader(-1),
        mSelectedVertexShaderPos(-1), mSelectedIndexBuffer(-1),
        mSelectedIndexBufferPos(-1), mSelectedVertexBuffer(-1),
        mSelectedVertexBufferPos(-1), mSelectedComputeShader(-1),
        mSelectedComputeShaderPos(-1), mSelectedGeometryShader(-1),
        mSelectedGeometryShaderPos(-1), mSelectedDomainShader(-1),
        mSelectedDomainShaderPos(-1), mSelectedHullShader(-1),
        mSelectedHullShaderPos(-1), mPinkingShader(0),

        hunting(HUNTING_MODE_DISABLED), fix_enabled(true),
        config_reloadable(false), show_original_enabled(false), huntTime(0),
        verbose_overlay(false), suppress_overlay(false),

        deferred_contexts_enabled(true),

        frame_analysis_registered(false), analyse_frame(false),
        analyse_frame_no(0), def_analyse_options(FrameAnalysisOptions::INVALID),
        cur_analyse_options(FrameAnalysisOptions::INVALID),

        shader_hash_type(ShaderHashType::FNV), texture_hash_version(0),
        EXPORT_SHADERS(false), EXPORT_HLSL(0), EXPORT_FIXED(false),
        EXPORT_BINARY(false), CACHE_SHADERS(false), DumpUsage(false),
        ENABLE_TUNE(false), gTuneStep(0.001f),

        iniParamsReserved(0),

        constants_run(false), frame_no(0), hWnd(NULL), hide_cursor(false),
        cursor_upscaling_bypass(true), check_foreground_window(false),

        GAME_INTERNAL_WIDTH(
            1), // it gonna be used by mouse pos hook in case of softwaremouse
                // is on and it can be called before
        GAME_INTERNAL_HEIGHT(
            1), //  the swap chain is created and the proper data set to avoid
                //  errors in the hooked winapi functions
        SCREEN_WIDTH(-1), SCREEN_HEIGHT(-1), SCREEN_REFRESH(-1),
        SCREEN_FULLSCREEN(0), SCREEN_ALLOW_COMMANDS(false),
        ...
        marking_actions(MarkingAction::INVALID), gForceStereo(0),
        gCreateStereoProfile(false), gSurfaceCreateMode(-1),
        gSurfaceSquareCreateMode(-1), gForceNoNvAPI(false),
        ZBufferHashToInject(0), SCISSOR_DISABLE(0),

        load_library_redirect(2), enable_hooks(EnableHooks::INVALID),
        enable_check_interface(false), enable_create_device(0),
        enable_platform_update(false), allow_swapchain2_passthrough(false),
        allow_swapchain3_passthrough(
            true), // Default enabled to fix modern games
        allow_swapchain4_passthrough(false), gInitialized(false),
        bIntendedTargetExe(false), gReloadConfigPending(false),
        gWipeUserConfig(false), user_config_dirty(0), gLogInput(false),
...
struct TLS {
  ...
  TLS() : hooking_quirk_protection(false) {}
};
...
static inline ShaderMap::iterator
lookup_shader_hash(ID3D11DeviceChild *shader) {
  return Profiling::lookup_map(G->mShaders, shader,
                               &Profiling::shader_hash_lookup_overhead);
}

static inline ShaderReloadMap::iterator
lookup_reloaded_shader(ID3D11DeviceChild *shader) {
  return Profiling::lookup_map(G->mReloadedShaders, shader,
                               &Profiling::shader_reload_lookup_overhead);
}

static inline ShaderReplacementMap::iterator
lookup_original_shader(ID3D11DeviceChild *shader) {
  return Profiling::lookup_map(G->mOriginalShaders, shader,
                               &Profiling::shader_original_lookup_overhead);
}

static inline ShaderOverrideMap::iterator lookup_shaderoverride(UINT64 hash) {
  return Profiling::lookup_map(G->mShaderOverrideMap, hash,
                               &Profiling::shaderoverride_lookup_overhead);
}

static inline ResourceMap::iterator
lookup_resource_handle_info(ID3D11Resource *resource) {
  return Profiling::lookup_map(G->mResources, resource,
                               &Profiling::texture_handle_info_lookup_overhead);
}

static inline TextureOverrideMap::iterator
lookup_textureoverride(uint32_t hash) {
  return Profiling::lookup_map(G->mTextureOverrideMap, hash,
                               &Profiling::textureoverride_lookup_overhead);
}
